;(function(){
try{
if(window.__gpActionsSync20260618Full91)return;
window.__gpActionsSync20260618Full91=true;
...
function syncFavoriteOverlays(){
  ...
  [].slice.call(document.querySelectorAll("main article")).forEach(function(card){
    ...
    if(isFavoritesPage&&!active&&card.dataset.gpFavoriteRemovedByUser90!=="true"){
      active=true;
      forcedFavoriteActive=true;
    }
    if(isFavoritesPage){
...
function gpNormalizeFavoritesCardHearts90(){
  ...
  [].slice.call(document.querySelectorAll("main .gp-favorites-storage-card,main article")).forEach(function(card){
    ...
    if(storageCard&&!localActive){
      card.style.setProperty("display","none","important");
      return;
    }
    if(!localActive&&card.dataset.gpFavoriteRemovedByUser90==="true")return;
    var visible=gpFavoriteCardVisible90(card);
    if(!localActive&&!storageCard&&!visible)return;
    card.style.removeProperty("display");
    gpEnsureFavoriteCardHeart90(card,id,!localActive);
...
function ensureFavoritesCard90Style(){
  if(document.getElementById("gp-favorites-card91-style"))return;
  var style=document.createElement("style");
  style.id="gp-favorites-card91-style";
  style.textContent=[
...
function run(){
  ...
  if(isCompare||isCart||isFavorites){
    gpRenderStoredActionPage();
    gpNormalizeVisibleProductPrices();
    if(isFavorites)gpNormalizeFavoritesCardHearts90();
  }
