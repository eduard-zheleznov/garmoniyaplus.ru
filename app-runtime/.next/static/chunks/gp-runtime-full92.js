;(function(){
try{
if(window.__gpActionsSync20260618Full92)return;
window.__gpActionsSync20260618Full92=true;
var gpFavoritesRoute92=location.search.indexOf("favorites=1")>-1;
if(gpFavoritesRoute92){
  document.documentElement.classList.add("gp-favorites-route92");
  if(document.body)document.body.classList.add("gp-favorites-route92");
}
  ...
  [].slice.call(document.querySelectorAll("main article")).forEach(function(card){
    ...
    var favoriteHandler=function(event){
      ...
      gpEnforceToggleState("lumera_favorites","gp_favorite_ids_v1",currentId,favoriteDesired,function(){
        ...
        gpSyncHeaderActionBadges();
        gpRenderStoredActionPage();
        gpNormalizeFavoritesCardHearts90();
      });
    };
...
function gpNormalizeFavoritesCardHearts90(){
  if(location.search.indexOf("favorites=1")<0)return;
  ensureFavoritesCard90Style();
  var favorites=gpActionIds("favorites");
...
function gpStrictMoneyMatches(value){
  ...
  var matches=[];
  text.replace(/(^|[^\d])((?:\d{1,3}(?:[ \t\u00a0\u202f]\d{3})+|\d+|0)[ \t\u00a0\u202f]*(?:в‚Ѕ|СЂСѓР±\.?|РІвЂљР…))/gi,function(_,prefix,price){
    matches.push(gpNormalizeMoneySpaces(price).replace(/РІвЂљР…/i,"в‚Ѕ"));
    return _;
  });
  return matches;
}
...
function gpNormalizePriceText(value){
  ...
  if(!/(в‚Ѕ|СЂСѓР±\.?|РІвЂљР…)/i.test(raw))return "";
  ...
  var glued=price.match(/^(\d{3,6})(\d[ \t\u00a0\u202f]\d{3}[ \t\u00a0\u202f]*(?:в‚Ѕ|СЂСѓР±\.?|РІвЂљР…))$/i);
  if(glued)price=glued[2];
  var spaced=price.match(/^\d{3,6}[ \t\u00a0\u202f]+(\d{1,3}(?:[ \t\u00a0\u202f]\d{3})+[ \t\u00a0\u202f]*(?:в‚Ѕ|СЂСѓР±\.?|РІвЂљР…))$/i);
  if(spaced)price=spaced[1];
  var digits=price.replace(/[^\d]/g,"");
  if(digits.length>6){
    var tail4=digits.slice(-4);
    if(Number(tail4)>=100)price=tail4.slice(0,1)+" "+tail4.slice(1)+" в‚Ѕ";
  }
  return gpNormalizeMoneySpaces(price).replace(/РІвЂљР…/i,"в‚Ѕ");
}
...
function gpRenderStoredActionPage(){
  ...
  if(kind==="favorites"){
    ...
    gpHideFavoritesCatalogChrome92();
  }
  document.body.classList.toggle("gp-compare-storage-ready",kind==="compare");
  document.body.classList.toggle("gp-favorites-storage-ready",kind==="favorites");
...
function normalizeFavoritesSimplePage(){
  ...
  [].slice.call(main.querySelectorAll("section,div,nav")).forEach(function(node){
    var text=(node.textContent||"").trim();
    if(/^РЎС‚СЂР°РЅРёС†Р°\s+\d+\s+РёР·\s+\d+/i.test(text)||(/РќР°Р·Р°Рґ/i.test(text)&&/Р”Р°Р»РµРµ/i.test(text)&&/\b1\b/.test(text))){
  ...
  if(!header){
    ...
    header.innerHTML='<h1>РР·Р±СЂР°РЅРЅРѕРµ</h1><p>РўРѕРІР°СЂС‹, РєРѕС‚РѕСЂС‹Рµ РІС‹ РґРѕР±Р°РІРёР»Рё РІ РёР·Р±СЂР°РЅРЅРѕРµ.</p>';
...
function gpHideFavoritesCatalogChrome92(){
  if(location.search.indexOf("favorites=1")<0)return;
  var main=document.querySelector("main");
  if(!main)return;
  [].slice.call(main.querySelectorAll("nav,[aria-label*='pagination' i],[class*='pagination' i]")).forEach(function(node){
    if(node.closest&&node.closest("[data-gp-storage-action-page]"))return;
...
function ensureFavoritesCard90Style(){
  if(document.getElementById("gp-favorites-only92-style"))return;
  var style=document.createElement("style");
  style.id="gp-favorites-only92-style";
  style.textContent=[
    ...
    "html.gp-favorites-route92 body:not(.gp-favorites-storage-ready) main article{display:none!important;visibility:hidden!important}",
    "html.gp-favorites-route92 body:not(.gp-favorites-storage-ready) main nav,html.gp-favorites-route92 body:not(.gp-favorites-storage-ready) main [aria-label*='pagination' i],html.gp-favorites-route92 body:not(.gp-favorites-storage-ready) main [class*='pagination' i],body.gp-favorites-route92 [data-gp-favorites-catalog-chrome='true']{display:none!important}",
    "html.gp-favorites-route92 header a[href='/catalog'],html.gp-favorites-route92 header a[href='/catalog/all-products'],body.gp-favorites-route92 header a[href='/catalog'],body.gp-favorites-route92 header a[href='/catalog/all-products']{background:#fff!important;color:var(--brand-text)!important;border-color:rgba(255,255,255,.75)!important;box-shadow:none!important;outline:0!important}",
    "html.gp-favorites-route92 header a[href*='favorites=1'],body.gp-favorites-route92 header a[href*='favorites=1']{background:var(--brand-primary)!important;color:#fff!important;border-color:var(--brand-primary)!important}",
...
function run(){
  ...
  document.documentElement.classList.toggle("gp-favorites-route92",isFavorites);
  document.body.classList.toggle("gp-favorites-route92",isFavorites);
  ensureLayoutFavoritesFiltersPatch();
