;(function(){
try{
if(window.__gpActionsSync20260618AndroidFast84)return;
...
var gpCurrentCatalogCategories=[
  {href:"/catalog/m-kosmeticheskie-sredstva",title:"М+ Косметические средства"},
  {href:"/catalog/dezinfekciya",title:"ДЕЗИНФЕКЦИЯ"},
  {href:"/catalog/kraft-paket",title:"КРАФТ- ПАКЕТ"},
  {href:"/catalog/vsesezonnye-pilingi",title:"ВСЕСЕЗОННЫЕ пилинги"},
  {href:"/catalog/odnorazovaya-produkciya",title:"ОДНОРАЗОВАЯ продукция"},
  {href:"/catalog/sl-beauty",title:"SL-BEAUTY"},
  {href:"/catalog/aksessuary-dlya-kosmetologa",title:"АКСЕССУАРЫ для косметолога"},
  {href:"/catalog/shpricy-igly-kanyuli",title:"ШПРИЦЫ ,ИГЛЫ , КАНЮЛИ ,"},
  {href:"/catalog/akugel",title:"АКУГЕЛЬ"},
  {href:"/catalog/fillery",title:"ФИЛЛЕРЫ"},
  {href:"/catalog/arkana",title:"АРКАНА"},
  {href:"/catalog/mezo-bio-niti",title:"МЕЗО / БИО / НИТИ"},
  {href:"/catalog/krasiteli-dlya-brovey-i-resnic",title:"КРАСИТЕЛИ ДЛЯ БРОВЕЙ И РЕСНИЦ"},
  {href:"/catalog/beauty-style",title:"Beauty Style"},
  {href:"/catalog/sredstva-dlya-depilyacii",title:"СРЕДСТВА ДЛЯ ДЕПИЛЯЦИИ"},
  {href:"/catalog/martineks",title:"МАРТИНЕКС"},
  {href:"/catalog/kosmetika",title:"КОСМЕТИКА"},
  {href:"/catalog/parikmaherskaya",title:"ПАРИКМАХЕРСКАЯ"}
];
...
function productTitleFromCard(card){
  ...
  if(!raw)return"";
  raw=raw
    .replace(/\b(в сравнении|сравнить|в корзину|убрать|подробнее|быстрый просмотр)\b.*$/i,"")
    .replace(/\s*\d[\d\s.,]*\s*₽.*$/,"")
    .replace(/\s*цена.*$/i,"")
    .trim();
  return raw;
...
function hideEmptyPromotionShowcase(){
  ...
  [].slice.call(document.querySelectorAll("main section")).forEach(function(section){
    ...
    if(text.indexOf("Товары по спецусловиям")>-1&&text.indexOf("Сейчас нет опубликованных товаров")>-1&&!section.querySelector("article")){
...
function normalizeMobileCategoryHeader(){
  ...
  var hasBack=links.some(function(a){return /Назад\s+в:/i.test((a.textContent||"").trim())});
...
function syncFavoriteOverlays(){
  ...
  [].slice.call(document.querySelectorAll("main article")).forEach(function(card){
    ...
    var favoriteHandler=function(event){
      ...
      var favoriteDesired=gpActionIds("favorites").indexOf(currentId)<0;
      if(!nativeFavorite){
        event.preventDefault();
        event.stopPropagation();
        if(event.stopImmediatePropagation)event.stopImmediatePropagation();
  ...
  if(isFavoritesPage){
    if(!visibleCount&&!empty){
      ...
      if(host){
        ...
        empty.textContent="В избранном пока нет доступных товаров.";
...
function gpStrictMoneyMatches(value){
  ...
  var matches=[];
  text.replace(/(^|[^\d])((?:\d{1,3}(?:[ \t\u00a0\u202f]\d{3})+|\d+|0)[ \t\u00a0\u202f]*(?:₽|руб\.?|в‚Ѕ))/gi,function(_,prefix,price){
    matches.push(gpNormalizeMoneySpaces(price).replace(/в‚Ѕ/i,"₽"));
    return _;
  });
  return matches;
}
...
function gpNormalizePriceText(value){
  ...
  var glued=price.match(/^(\d{3,6})(\d[ \t\u00a0\u202f]\d{3}[ \t\u00a0\u202f]*(?:₽|руб\.?|в‚Ѕ))$/i);
  if(glued)price=glued[2];
  var spaced=price.match(/^\d{3,6}[ \t\u00a0\u202f]+(\d{1,3}(?:[ \t\u00a0\u202f]\d{3})+[ \t\u00a0\u202f]*(?:₽|руб\.?|в‚Ѕ))$/i);
  ...
  return gpNormalizeMoneySpaces(price).replace(/в‚Ѕ/i,"₽");
}
...
function gpProductSnapshotFromCard(card,id){
  ...
  var meta=lines.filter(function(line){
    ...
    return !/(избран|сравн|корзин|подробнее|купить|favorite|compare|cart)/i.test(line);
  }).slice(0,8);
  return{
    id:id,
...
function gpSaveProductSnapshot(id,card){
  ...
  map[id]=Object.assign({},map[id]||{},snapshot);
...
function gpFormatMoney(value){
  if(!value)return "";
  try{return new Intl.NumberFormat("ru-RU",{style:"currency",currency:"RUB",maximumFractionDigits:0}).format(value)}catch(error){return String(value)+" ₽"}
}
...
function gpActionProducts(kind,ids){
  ...
  return ids.map(function(id){
    ...
    return Object.assign({id:id,title:gpTitleFromId(id),href:"/catalog/all-products",image:"",price:"",meta:[]},snapshots[id]||{},{
      id:id,
...
function gpCompareValue(product,type){
  ...
  if(type==="price")return product.price||"—";
  if(type==="category")return product.category||meta[0]||"—";
  if(type==="description")return product.subtitle||meta.slice(1,3).join(" · ")||"—";
  if(type==="format")return meta.find(function(line){return /(\d+\s*(?:мл|ml|г|g|шт|амп|amp|%))/i.test(line)})||"—";
  if(type==="features")return meta.slice(2,5).join(" · ")||"—";
  return "—";
}
...
function gpProductCountLabel(count){
  ...
  return value+" "+gpPluralWord(value,"товар","товара","товаров");
}
...
function gpComparePageHtml(products,withCards){
  ...
  var head='<div class="grid gap-4 rounded-[1.6rem] bg-[var(--brand-surface)] text-left text-sm text-[var(--brand-muted)]" style="'+gridStyle+'"><div class="sticky left-0 z-20 bg-[var(--brand-surface)] px-5 py-4 font-medium shadow-[12px_0_24px_-18px_rgba(36,29,25,0.26)]">Параметры</div>'+products.map(function(product){return '<div class="px-5 py-4 font-medium">'+gpEscapeHtml(product.title||gpTitleFromId(product.id))+'</div>';}).join("")+'</div>';
  ...
  return '<div class="gp-native-action-fallback gp-native-compare-fallback"><div class="hidden overflow-x-auto pb-2 md:block"><div class="space-y-6" style="'+widthStyle+'"><div class="grid gap-4" style="'+gridStyle+'"><aside class="rounded-[2rem] border border-white/75 bg-white/82 p-5 shadow-[0_20px_60px_rgba(36,29,25,0.08)]"><div class="text-[11px] uppercase tracking-[0.22em] text-[var(--brand-primary)]">Сравнение</div><h2 class="mt-3 font-[family:var(--font-heading)] text-3xl leading-none text-[var(--brand-text)]">'+products.length+' '+(products.length===1?'товар':'товара')+'</h2></aside>'+cards+'</div><div class="rounded-[2rem] border border-white/75 bg-white/84 shadow-[0_20px_60px_rgba(36,29,25,0.08)]"><div class="space-y-0">'+head+body+'</div></div></div></div><div class="space-y-4 md:hidden"><div class="rounded-[1.4rem] border border-white/75 bg-white/84 p-3.5 shadow-[0_20px_60px_rgba(36,29,25,0.08)]"><div class="text-[11px] uppercase tracking-[0.22em] text-[var(--brand-primary)]">Сравнение</div><h2 class="mt-3 font-[family:var(--font-heading)] text-[2rem] leading-none text-[var(--brand-text)]">'+products.length+' '+(products.length===1?'товар':'товара')+'</h2></div><div class="overflow-x-auto rounded-[1.4rem] border border-white/75 bg-white/84 shadow-[0_20px_60px_rgba(36,29,25,0.08)]"><div class="min-w-full w-max"><div class="grid bg-[var(--brand-surface)] text-left text-sm text-[var(--brand-muted)]" style="'+gridStyle+'"><div class="sticky left-0 z-20 flex min-h-[11.8rem] items-end bg-[var(--brand-surface)] px-2 pb-2.5 pt-6 text-[11px] font-medium text-[var(--brand-text)] shadow-[10px_0_24px_-18px_rgba(36,29,25,0.22)]">Параметры</div>'+products.map(function(product){var image=product.image||"/editorial/hero-beauty.jpg";var title=product.title||gpTitleFromId(product.id);return '<div class="grid h-full grid-rows-[auto_minmax(2.65rem,auto)_auto] px-2 py-3"><a href="'+gpEscapeHtml(product.href||"/catalog/all-products")+'"><span class="relative block min-h-[4.8rem] overflow-hidden rounded-[1.25rem] bg-[var(--brand-surface)]"><img src="'+gpEscapeHtml(image)+'" alt="'+gpEscapeHtml(title)+'" loading="lazy" class="absolute inset-0 h-full w-full object-contain p-2"></span><span class="mt-2 block min-h-[2.65rem] text-[11px] font-medium leading-4 text-[var(--brand-text)]">'+gpEscapeHtml(title)+'</span></a><div class="mt-1 text-[11px] font-semibold text-[var(--brand-text)]">'+gpEscapeHtml(product.price||"")+'</div></div>';}).join("")+'</div>'+body+'</div></div></div></div>';
}
...
function gpCartPageHtml(products){
  ...
  var rows=products.map(function(product){
    ...
    return '<article class="grid gap-4 rounded-[2rem] border border-white/75 bg-white/84 p-5 shadow-[0_20px_60px_rgba(36,29,25,0.08)] md:grid-cols-[0.34fr_0.66fr]" data-gp-cart-product="'+gpEscapeHtml(product.id)+'"><a href="'+gpEscapeHtml(product.href||"/catalog/all-products")+'" class="relative block min-h-[14rem] overflow-hidden rounded-[2rem] border border-white/70 bg-gradient-to-br from-[#eadccf] via-[#f7f2ec] to-[#fffaf4] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.55)]"><img src="'+gpEscapeHtml(image)+'" alt="'+gpEscapeHtml(title)+'" loading="lazy" class="absolute inset-0 h-full w-full object-contain p-3 md:p-5"></a><div class="flex flex-col justify-between gap-4"><div><a class="font-[family:var(--font-heading)] text-3xl leading-none text-[var(--brand-text)]" href="'+gpEscapeHtml(product.href||"/catalog/all-products")+'">'+gpEscapeHtml(title)+'</a><p class="mt-3 text-sm leading-7 text-[var(--brand-muted)]">'+gpEscapeHtml(product.subtitle||product.category||"")+'</p></div><div class="flex flex-wrap items-center justify-between gap-3"><div class="inline-flex items-center gap-2 rounded-full border border-[var(--brand-primary)]/20 bg-[var(--brand-surface)] px-3 py-2"><button type="button" data-gp-cart-minus="'+gpEscapeHtml(product.id)+'" class="rounded-full p-1" aria-label="Уменьшить">−</button><span class="min-w-8 text-center text-sm font-medium text-[var(--brand-text)]">'+qty+'</span><button type="button" data-gp-cart-plus="'+gpEscapeHtml(product.id)+'" class="rounded-full p-1" aria-label="Увеличить">+</button></div><div class="text-xl font-semibold text-[var(--brand-text)]">'+gpEscapeHtml(lineTotal?gpFormatMoney(lineTotal):price)+'</div><button type="button" class="inline-flex items-center gap-2 rounded-full border border-rose-200 px-4 py-2 text-sm text-rose-600" data-gp-cart-remove="'+gpEscapeHtml(product.id)+'">Удалить</button></div></div></article>';
  }).join("");
  return '<div class="gp-native-action-fallback gp-native-cart-fallback hidden gap-6 md:grid lg:grid-cols-[1.2fr_0.8fr]"><div class="space-y-4">'+rows+'</div><aside class="rounded-[2rem] border border-white/75 bg-white/84 p-6 shadow-[0_20px_60px_rgba(36,29,25,0.08)]"><div class="text-xs uppercase tracking-[0.22em] text-[var(--brand-primary)]">Итого</div><div class="mt-3 font-[family:var(--font-heading)] text-5xl leading-none text-[var(--brand-text)]">'+(total?gpEscapeHtml(gpFormatMoney(total)):"по запросу")+'</div><p class="mt-3 text-sm leading-7 text-[var(--brand-muted)]">Доставку и подтверждение заказа менеджер согласует после оформления.</p><button type="button" data-gp-open-contact class="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[var(--brand-primary)] px-5 py-3.5 text-sm font-semibold text-white shadow-[0_18px_35px_rgba(175,90,102,0.24)] transition hover:-translate-y-0.5">Перейти к оформлению</button></aside></div><div class="gp-native-action-fallback gp-native-cart-fallback-mobile space-y-3 md:hidden">'+rows+'<div class="rounded-[1.55rem] border border-white/75 bg-white/84 p-4 shadow-[0_20px_60px_rgba(36,29,25,0.08)]"><div class="flex items-center justify-between text-sm font-semibold text-[var(--brand-text)]"><span>Итого</span><span>'+(total?gpEscapeHtml(gpFormatMoney(total)):"по запросу")+'</span></div><button type="button" data-gp-open-contact class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[var(--brand-primary)] px-5 py-3 text-sm font-semibold text-white shadow-[0_18px_35px_rgba(175,90,102,0.24)] transition hover:-translate-y-0.5">Оформить</button></div></div>';
}
...
function gpHideEmptyActionBlocks(main,kind){
  var patterns=kind==="cart"?[/Корзина пуста/i,/Перейдите в каталог/i,/РљРѕСЂР·РёРЅР° РїСѓСЃС‚Р°/i]:[/Список сравнения пуст/i,/Добавьте товары из каталога/i,/нет товаров/i,/пока нет/i,/РЅРµС‚ С‚РѕРІР°СЂРѕРІ/i];
...
function gpRenderStoredActionPage(){
  ...
  var hasCompareParams=kind==="compare"&&(main.querySelector("table")||/Тип кожи|Текстура|Ингреди|Задач|Параметр|Объ[её]м/i.test(text));
  var hasCheckout=kind==="cart"&&(/Оформить|заказ|Имя|Телефон|Доставка/i.test(text)&&main.querySelector("form,textarea,input"));
...
function gpOrderSummaryText(form){
  ...
  return ["Заказ с сайта ГармонияПлюс",name?"Имя: "+name:"",phone?"Телефон: "+phone:"","Товары:",lines.join("\n"),message?"Комментарий: "+message:""].filter(Boolean).join("\n");
}
...
function installStoredActionPageHandlers(){
  ...
  document.addEventListener("click",function(event){
    ...
    if(compareFavorite||compareCart||compareToggle){
      ...
      if(compareFavorite){
        var favoriteActive=gpUnionIds("lumera_favorites","gp_favorite_ids_v1").indexOf(actionId)<0;
  ...
  document.addEventListener("submit",function(event){
    ...
    if(status){
      status.textContent="Заявка подготовлена. Отправьте ее в чат или дождитесь звонка менеджера.";
...
function gpEnsureDesktopCompareLink(count){
  ...
  link.innerHTML=gpDesktopCompareIcon()+'<span class="pointer-events-none absolute left-1/2 top-[calc(100%+0.7rem)] hidden -translate-x-1/2 whitespace-nowrap rounded-full bg-[var(--brand-text)] px-3 py-1 text-[11px] font-medium text-white opacity-0 transition group-hover:opacity-100 lg:block">Сравнение</span>';
...
window.__gpNativeProductActionClick=function(event,button){
  try{
    ...
    if(kind==="cart"){
      ...
      gpShowActionToast("Товар добавлен в корзину");
    }
...
function gpCardActionButtonState(){
  ...
  [].slice.call(document.querySelectorAll("main article")).forEach(function(card){
    ...
    [].slice.call(card.querySelectorAll("button")).forEach(function(button){
      ...
      if(label.indexOf("сравнен")>-1||label.indexOf("сравнить")>-1||label.indexOf("сравнение")>-1){
        button.classList.toggle("text-rose-600",inCompare);
        button.classList.toggle("border-rose-200",inCompare);
        button.title=inCompare?"Убрать из сравнения":"Добавить в сравнение";
        button.setAttribute("aria-label",button.title);
      }
      if(label.indexOf("корзин")>-1){
        button.classList.toggle("ring-2",inCart);
        button.classList.toggle("ring-[var(--brand-primary)]/35",inCart);
        button.classList.toggle("gp-cart-added",inCart);
        button.dataset.gpCartAdded=inCart?"true":"false";
        button.title=inCart?"В корзине":"В корзину";
        button.setAttribute("aria-label",button.title);
      }
    });
  });
...
function gpRenderSearchSuggestions(input,items,query){
  ...
  if(!matches.length){
    box.innerHTML='<div class="gp-search-empty">Ничего не найдено</div>';
    return;
  }
  box.innerHTML=matches.map(function(match){
    var kind=match.item.kind==="category"?"Категория":"Товар";
...
function installSearchSuggestions(){
  [].slice.call(document.querySelectorAll('input[type="search"],input[placeholder*="Поиск"],input[placeholder*="РџРѕРёСЃРє"]')).forEach(function(input){
  ...
  document.addEventListener("click",function(event){
    ...
    if(!event.target.closest||!event.target.closest(".gp-search-suggestions")&&!event.target.closest('input[type="search"],input[placeholder*="Поиск"],input[placeholder*="РџРѕРёСЃРє"]')){
...
function gpFindVisibleSearchInput(){
  return [].slice.call(document.querySelectorAll('input[type="search"],input[placeholder*="Поиск"],input[placeholder*="РџРѕРёСЃРє"]')).find(function(input){
...
function gpOpenMobileSearchFallback(){
  ...
  root.innerHTML='<div class="gp-mobile-search-dialog" role="dialog" aria-modal="true" aria-label="Поиск"><div class="gp-mobile-search-top"><div class="gp-mobile-search-field"><input class="gp-mobile-search-input" type="search" placeholder="Поиск по каталогу" autocomplete="off"></div><button class="gp-mobile-search-close" type="button" aria-label="Закрыть поиск">×</button></div></div>';
...
function installMobileSearchFallback(){
  ...
  document.addEventListener("click",function(event){
    ...
    if(!/Поиск|РџРѕРёСЃРє|Search/i.test(label))return;
    setTimeout(function(){
...
function gpLooksLikeContactTrigger(node){
  ...
  return /связ|чат|консульт|whats|telegram|max|способы|РѕРЅР»Р°Р№РЅ|РєРѕРЅСЃСѓР»СЊС‚/i.test(label);
}
...
function gpVisibleNativeContactPanel(){
  return [].slice.call(document.querySelectorAll("form,section,div")).some(function(node){
    ...
    if(!/Напишите|Онлайн-консультация|WhatsApp|Telegram|Max|РќР°РїРёС€РёС‚Рµ|РћРЅР»Р°Р№РЅ/i.test(text))return false;
...
function ensureContactWidgetFallback(forceOpen,message){
  ...
  if(!root){
    ...
    root.innerHTML='<div class="gp-contact-widget-panel"><h3>Напишите нам</h3><p>Поможем с подбором, заказом и доставкой.</p><div class="gp-contact-widget-links"><a href="tel:+79134487333">Позвонить <span>+7 913 448-73-33</span></a><a href="https://wa.me/79134487333" target="_blank" rel="noreferrer">WhatsApp <span>Открыть</span></a><a href="/contacts">Контакты <span>Все способы</span></a></div><form data-gp-contact-form><input name="name" placeholder="Ваше имя"><input name="phone" inputmode="tel" placeholder="+7 (___) ___-__-__"><textarea name="message" placeholder="Ваш вопрос"></textarea><button type="submit">Отправить запрос</button><p class="gp-order-status" data-gp-contact-status></p></form></div><button class="gp-contact-widget-main" type="button" aria-label="Открыть способы связи"><span class="gp-contact-widget-icon is-active">'+gpContactSvg("chat")+'</span><span class="gp-contact-widget-icon">'+gpContactSvg("max")+'</span><span class="gp-contact-widget-icon">'+gpContactSvg("phone")+'</span><span class="gp-contact-widget-icon">'+gpContactSvg("send")+'</span></button>';
    ...
    root.querySelector("[data-gp-contact-form]").addEventListener("submit",function(event){
      ...
      if(status){
        status.textContent="Запрос подготовлен. Менеджер свяжется с вами по указанному телефону.";
        status.classList.add("is-visible");
      }
    });
    document.body.appendChild(root);
...
function gpSuppressNativeContactWidget73(root){
  try{
    [].slice.call(document.querySelectorAll("button[aria-label]")).forEach(function(button){
      ...
      if(!/(связ|консульт|contact|chat|способы)/i.test(label+" "+text))return;
...
function normalizeFavoritesSimplePage(){
  ...
  var main=document.querySelector("main");
  if(!main)return;
  [].slice.call(main.querySelectorAll(".gp-favorites-product-grid,.gp-favorites-grid-shell")).forEach(function(node){
    node.classList.remove("gp-favorites-product-grid","gp-favorites-grid-shell");
  });
  [].slice.call(main.querySelectorAll(".gp-favorites-hidden")).forEach(function(node){
    if(node.querySelector&&node.querySelector("article")){
      node.classList.remove("gp-favorites-hidden");
      node.style.removeProperty("display");
    }
  });
  [].slice.call(main.querySelectorAll("section")).forEach(function(section){
    var hasArticle=!!section.querySelector("article");
    var nested=!!section.querySelector("section article");
    if(!hasArticle&&!section.classList.contains("gp-favorites-simple-header")){
      section.dataset.gpHideOnFavorites="true";
      section.style.setProperty("display","none","important");
    }
    if(hasArticle&&nested){
      section.classList.add("gp-favorites-grid-shell");
      section.classList.remove("gp-favorites-hidden");
      section.style.setProperty("display","contents","important");
    }
  });
  var firstArticle=main.querySelector("article");
  var productGrid=firstArticle&&firstArticle.parentElement;
  if(productGrid){
    productGrid.classList.add("gp-favorites-product-grid");
    productGrid.classList.remove("gp-favorites-hidden");
    productGrid.style.setProperty("display","grid","important");
    productGrid.style.setProperty("grid-template-columns",innerWidth>=1024?"repeat(4, minmax(0, 1fr))":"repeat(2, minmax(0, 1fr))","important");
    productGrid.style.setProperty("gap",innerWidth>=1024?"1.25rem":".75rem","important");
    var node=productGrid.parentElement;
    while(node&&node!==main){
      if(node.querySelector&&node.querySelector("article")){
        node.classList.add("gp-favorites-grid-shell");
        node.classList.remove("gp-favorites-hidden");
  ...
  [].slice.call(main.querySelectorAll("nav,[aria-label*='pagination' i],[class*='pagination' i]")).forEach(function(node){node.classList.add("gp-favorites-hidden")});
  [].slice.call(main.querySelectorAll("section,div,nav")).forEach(function(node){
    var text=(node.textContent||"").trim();
    if(/^Страница\s+\d+\s+из\s+\d+/i.test(text)||(/Назад/i.test(text)&&/Далее/i.test(text)&&/\b1\b/.test(text))){
      node.classList.add("gp-favorites-hidden");
      node.style.setProperty("display","none","important");
    }
  });
  var header=main.querySelector(".gp-favorites-simple-header");
  if(!header){
    header=document.createElement("section");
    header.className="gp-favorites-simple-header";
    header.innerHTML='<h1>Избранное</h1><p>Товары, которые вы добавили в избранное.</p>';
...
function normalizeFavoritesNavActive(){
  ...
  [].slice.call(document.querySelectorAll("header a,header button,nav a,nav button")).forEach(function(node){
    var text=(node.textContent||node.getAttribute("aria-label")||"").trim();
    if(text==="Каталог"){
      node.style.setProperty("background","white","important");
      node.style.setProperty("color","var(--brand-text)","important");
      node.style.setProperty("box-shadow","none","important");
      node.style.setProperty("border-color","rgba(175,90,102,.14)","important");
...
function ensureDesktopFallbackFilters(){
  ...
  if(/ФИЛЬТРЫ|Фильтры|Категория|Товаров в ряд/.test(document.body.textContent||""))return;
  ...
  aside.innerHTML=[
    ...
    '<div class="mb-4"><div class="text-xs uppercase tracking-[0.24em] text-[var(--brand-primary)]">Фильтры</div><p class="mt-2 text-sm leading-6 text-[var(--brand-muted)]">Выберите параметры.</p></div>',
    '<section class="mb-3 rounded-[1.35rem] border border-white/70 bg-white/72 p-3.5"><div class="mb-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--brand-muted)]">Категория</div><select data-gp-filter-category class="field py-2.5 text-sm"><option value="/catalog/all-products">Все товары</option></select></section>',
    '<section class="mb-3 rounded-[1.35rem] border border-white/70 bg-white/72 p-3.5"><div class="mb-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--brand-muted)]">Товаров в ряд</div><div class="flex gap-2"><button data-gp-cols="2" class="gp-filter-pill">2</button><button data-gp-cols="3" class="gp-filter-pill is-active">3</button><button data-gp-cols="4" class="gp-filter-pill">4</button></div></section>',
    '<section class="mb-3 rounded-[1.35rem] border border-white/70 bg-white/72 p-3.5"><div class="mb-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--brand-muted)]">Товаров на странице</div><div class="flex flex-wrap gap-2"><button data-gp-page-size="8" class="gp-filter-pill">8</button><button data-gp-page-size="12" class="gp-filter-pill is-active">12</button><button data-gp-page-size="16" class="gp-filter-pill">16</button><button data-gp-page-size="24" class="gp-filter-pill">24</button></div></section>',
    '<section class="mb-3 rounded-[1.35rem] border border-white/70 bg-white/72 p-3.5"><div class="mb-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--brand-muted)]">Сортировка</div><div class="space-y-2"><button data-gp-sort="sale" class="gp-filter-line">Сначала акции</button><button data-gp-sort="new" class="gp-filter-line">Сначала новинки</button><button data-gp-sort="price-asc" class="gp-filter-line">Цена по возрастанию</button><button data-gp-sort="price-desc" class="gp-filter-line">Цена по убыванию</button></div></section>',
    '<section class="rounded-[1.35rem] border border-white/70 bg-white/72 p-3.5"><div class="mb-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--brand-muted)]">Цена</div><div class="space-y-2"><button data-gp-price="" class="gp-filter-line">Все цены</button><button data-gp-price="under-4000" class="gp-filter-line">До 4 000 ₽</button><button data-gp-price="4000-6000" class="gp-filter-line">4 000-6 000 ₽</button><button data-gp-price="over-6000" class="gp-filter-line">Более 6 000 ₽</button></div></section>',
    '</div>'
  ].join("");
...
function gpOldFilterSection(title,inner,open){
  return '<section class="gp-old-filter-section'+(open?' is-open':'')+'"><button type="button" class="gp-old-filter-section-head" data-gp-toggle-section><span>'+title+'</span><span aria-hidden="true">⌄</span></button><div class="gp-old-filter-section-body">'+inner+'</div></section>';
}
...
function gpBuildOldFilterPanel(mobile){
  ...
  var content=[
    '<div class="gp-old-filter-head"><div><div class="gp-old-filter-kicker">Фильтры</div><p>Выберите параметры.</p><div class="gp-old-filter-count">Найдено: '+(count||document.querySelectorAll("main article").length)+'</div></div>'+(mobile?'<button type="button" class="gp-old-filter-close" data-gp-close-filters aria-label="Закрыть фильтры">×</button>':'<button type="button" class="gp-old-filter-close" data-gp-collapse-filters aria-label="Скрыть фильтры">×</button>')+'</div>',
    '<section class="gp-old-filter-box"><div class="gp-old-filter-label">Категория</div><label><span class="sr-only">Выберите категорию</span><select data-gp-old-category class="field py-2.5 text-sm"><option value="/catalog/all-products">Все товары</option></select></label></section>',
    mobile?'':('<section class="gp-old-filter-box"><div class="gp-old-filter-label">Товаров в ряд</div><div class="gp-old-filter-row">'+gpFilterButton('data-gp-cols="2"','2',cols==="2")+gpFilterButton('data-gp-cols="3"','3',cols==="3")+gpFilterButton('data-gp-cols="4"','4',cols==="4")+'</div></section>'),
    mobile?'':('<section class="gp-old-filter-box"><div class="gp-old-filter-label">Товаров на странице</div><div class="gp-old-filter-row">'+gpFilterButton('data-gp-page-size="8"','8',pageSize==="8")+gpFilterButton('data-gp-page-size="12"','12',pageSize==="12")+gpFilterButton('data-gp-page-size="16"','16',pageSize==="16")+gpFilterButton('data-gp-page-size="24"','24',pageSize==="24")+'</div></section>'),
    gpOldFilterSection('Сортировка',[
      gpOldFilterLine('data-gp-sort="sale"','Сначала акции',sort==="sale"),
      gpOldFilterLine('data-gp-sort="new"','Сначала новинки',sort==="new"),
      gpOldFilterLine('data-gp-sort="price-asc"','Цена по возрастанию',sort==="price-asc"),
      gpOldFilterLine('data-gp-sort="price-desc"','Цена по убыванию',sort==="price-desc")
    ].join(''),false),
    gpOldFilterSection('Акценты',[
      gpOldFilterLine('data-gp-special=""','Все акценты',!special),
      gpOldFilterLine('data-gp-special="sale"','Со скидкой',special==="sale"),
      gpOldFilterLine('data-gp-special="new"','Новинки',special==="new"),
      gpOldFilterLine('data-gp-special="featured"','Бестселлеры',special==="featured")
    ].join(''),false),
    gpOldFilterSection('Цена',[
      gpOldFilterLine('data-gp-price=""','Все цены',!price),
      gpOldFilterLine('data-gp-price="under-4000"','До 4 000 ₽',price==="under-4000"),
      gpOldFilterLine('data-gp-price="4000-6000"','4 000-6 000 ₽',price==="4000-6000"),
      gpOldFilterLine('data-gp-price="over-6000"','Более 6 000 ₽',price==="over-6000")
    ].join(''),false),
    gpOldFilterSection('Тип кожи',gpOldFilterLine('data-gp-skin=""','Любой тип',!params.get("skin")),false),
    gpOldFilterSection('Текстура',gpOldFilterLine('data-gp-texture=""','Все текстуры',!params.get("texture")),false),
    gpOldFilterSection('Задача',gpOldFilterLine('data-gp-concern=""','Все задачи',!params.get("concern")),false),
    gpOldFilterSection('Ингредиенты',gpOldFilterLine('data-gp-ingredient=""','Все ингредиенты',!params.get("ingredient")),false),
    gpOldFilterSection('Формат',gpOldFilterLine('data-gp-volume=""','Любой объем',!params.get("volume")),false),
...
function updateOldFilterCounts(){
  ...
  var setCount=function(value){
    [].slice.call(document.querySelectorAll(".gp-old-filter-count")).forEach(function(node){node.textContent="Найдено: "+value});
    [].slice.call(document.querySelectorAll(".gp-old-mobile-header-actions span")).forEach(function(node){
      if(value>0)node.textContent="Найдено: "+value;else node.remove();
    });
...
function gpUpdateCatalogPagination(){
  ...
  var label="Страница "+current+" из "+total;
  ...
  [].slice.call(main.querySelectorAll("p,span,div")).forEach(function(node){
    ...
    if(/^Страница\s+\d+\s+из\s+\d+$/i.test(text))node.textContent=label;
  });
  [].slice.call(main.querySelectorAll("a,button")).forEach(function(node){
    ...
    if(/Далее|Next/i.test(text)&&current>=total){
...
function gpFindCatalogPaginationShell(node){
  ...
  while(current&&current!==document.body){
    ...
    if(/Страница\s+\d+\s+из\s+\d+/i.test(text)&&/Назад/i.test(text)&&/Далее/i.test(text))return current;
...
function gpHandleCatalogPaginationClick(event){
  ...
  var match=((shell.textContent||"").replace(/\s+/g," ").trim()).match(/Страница\s+(\d+)\s+из\s+(\d+)/i);
  ...
  var next=current;
  if(/Назад/i.test(label))next=current-1;
  else if(/Далее|Next/i.test(label))next=current+1;
...
function updateCategoryHeaderCount(value){
  ...
  pill.textContent="Найдено: "+count;
}
  ...
  [].slice.call(document.querySelectorAll('main a[href^="/catalog/"]')).forEach(function(card){
    ...
    if(!blocks.length){
      ...
      if(raw){
        ...
        fallbackCount.textContent=((raw.match(/(\d+\s*(?:товар|товара|товаров|product[^\s]*)[^]*)/i)||[])[1])||raw;
        textWrap.appendChild(fallbackCount);
        blocks=[fallbackCount];
      }
    }
    var countBlock=blocks.find(function(node){
      return /\d/.test((node.textContent||""))&&/(товар|товара|товаров|product)/i.test(gpNormalizeText(node.textContent||""));
...
function openOldMobileFilters(){
  ...
  overlay.innerHTML='<button type="button" class="gp-old-filter-backdrop" data-gp-close-filters aria-label="Закрыть фильтры"></button><div class="gp-old-filter-mobile-shell">'+gpBuildOldFilterPanel(true)+'</div>';
  ...
  if(innerWidth>=1024){
    ...
    if(!header){
      ...
      if(container&&!container.querySelector(".gp-old-mobile-catalog-header")){
        var title=(document.querySelector("main h1")&&document.querySelector("main h1").textContent)||"Все товары";
        var count=document.querySelectorAll("main article").length;
        header=document.createElement("section");
        header.className="gp-old-mobile-catalog-header";
        header.innerHTML='<div class="gp-old-mobile-header-actions"><a href="/catalog">Все категории</a>'+(count>0?'<span>Найдено: '+count+'</span>':'')+'</div><h1>'+title+'</h1>';
...
var gpCompanyDetails=[
  ["ООО","ООО \"Гармония Плюс\""],
  ["Юридический адрес","655017, Республика Хакасия, г. Абакан, ул. Торосово, 9а"],
  ["ИНН","1901084417"],
  ["ОГРН","1081901002028"],
  ["КПП","190101001"],
  ["Р/С","40702810271000004510"],
  ["Банк","АБАКАНСКОЕ ОТДЕЛЕНИЕ № 8602 ПАО СБЕРБАНК, г. Абакан"],
  ["К/С","30101810500000000608"],
  ["БИК","049514608"],
  ["Телефон","8-913-448-7333, 8-983-191-03-83"],
  ["Email","Abakan-sib@mail.ru"],
  ["Налоговый режим","УСН доход - расход, НДС 5%"],
  ["Директор","Чаптыков Андрей Григорьевич"]
];

var gpLegalDocuments={
  payment:{
    title:"Оплата",
    intro:"Заказы на сайте garmoniyaplus.ru оплачиваются в рублях РФ. После оформления заказа менеджер подтверждает наличие товаров, итоговую сумму и доступный способ оплаты.",
    sections:[
      ["Способы оплаты",["Банковской картой онлайн через платежный сервис ЮKassa после подключения онлайн-оплаты.","Безналичным переводом на расчетный счет ООО \"Гармония Плюс\" по согласованным реквизитам.","Иной способ оплаты может быть согласован с менеджером до передачи заказа."]],
      ["Порядок оплаты",["Оплата производится после проверки состава заказа и подтверждения продавцом.","Электронный чек направляется покупателю на указанный email или телефон, если это предусмотрено выбранным способом оплаты.","При ошибке оплаты заказ не считается оплаченным; покупатель может повторить оплату или связаться с продавцом."]]
    ]
  },
  delivery:{
    title:"Доставка",
    intro:"Доставка согласуется с покупателем после оформления заказа и подтверждения наличия товаров.",
    sections:[
      ["Условия доставки",["Доставка возможна по г. Абакану, Республике Хакасия и в другие регионы РФ по согласованию.","Способ доставки, сроки и стоимость зависят от адреса, состава заказа и выбранной службы доставки.","Итоговая стоимость доставки сообщается покупателю до оплаты или передачи заказа."]],
      ["Получение заказа",["При получении проверьте целостность упаковки и соответствие заказа.","Если товар поврежден или состав заказа не совпадает, сообщите продавцу в день получения."]]
    ]
  },
  returns:{
    title:"Возврат",
    intro:"Возврат и обмен товаров выполняются в соответствии с законодательством РФ и условиями продажи.",
    sections:[
      ["Возврат товара надлежащего качества",["Возврат возможен, если товар не был в употреблении, сохранены товарный вид, упаковка, потребительские свойства и документы о покупке.","Для отдельных категорий косметической, парфюмерной, санитарно-гигиенической и медицинской продукции возврат товара надлежащего качества может быть ограничен законодательством РФ."]],
      ["Возврат товара ненадлежащего качества",["Если обнаружен брак, повреждение или ошибка комплектации, покупатель может обратиться к продавцу по телефону или email.","После проверки обращения продавец согласует замену, возврат товара или возврат денежных средств.","Деньги возвращаются тем же способом, которым была произведена оплата, если стороны не согласовали иной порядок."]]
    ]
  },
  privacy:{
    title:"Политика конфиденциальности",
    intro:"ООО \"Гармония Плюс\" обрабатывает персональные данные пользователей сайта garmoniyaplus.ru для оформления заказов, обратной связи, доставки и исполнения требований законодательства.",
    sections:[
      ["Какие данные обрабатываются",["ФИО, телефон, email, адрес доставки, состав заказа, сообщения покупателя и технические данные посещения сайта.","Данные платежей обрабатываются платежным сервисом и банками; сайт не хранит полные данные банковских карт."]],
  ...
  offer:{
    ...
    intro:"Настоящая оферта определяет условия продажи товаров через сайт garmoniyaplus.ru. Продавец - ООО \"Гармония Плюс\".",
    sections:[
      ["Предмет оферты",["Продавец обязуется передать покупателю товары, представленные на сайте, а покупатель обязуется принять и оплатить заказ.","Информация о товаре, цене и наличии размещается в карточках товаров и уточняется при подтверждении заказа."]],
      ["Оформление заказа",["Заказ оформляется через корзину сайта или через обращение к продавцу.","Заказ считается принятым после подтверждения продавцом состава, наличия, цены, оплаты и доставки."]],
      ["Оплата, доставка и возврат",["Условия оплаты, доставки и возврата указаны в соответствующих разделах сайта и являются частью настоящей оферты.","Все расчеты производятся в рублях РФ."]]
    ]
  }
};
...
function gpLegalHtml(doc){
  ...
  html+='<section><h3>Реквизиты продавца</h3>'+gpDetailsList()+'</section>';
  return html;
}
...
function openGpLegalModal(key){
  ...
  root.innerHTML='<div class="gp-legal-backdrop" data-gp-close="1"></div><div class="gp-legal-dialog" role="dialog" aria-modal="true" aria-label="'+doc.title+'"><button class="gp-legal-close" type="button" aria-label="Закрыть" data-gp-close="1">×</button><div class="gp-legal-kicker">Документ</div><h2>'+doc.title+'</h2><div class="gp-legal-content">'+gpLegalHtml(doc)+'</div></div>';
  root.addEventListener("click",function(event){
...
function ensureLegalFooterButtons(){
  ...
  var existing=[].slice.call(footer.querySelectorAll("button")).filter(function(button){
    return /Политика конфиденциальности|Согласие на обработку данных/i.test((button.textContent||"").trim());
  });
...
function installLegalDocumentHandlers(){
  ...
  document.addEventListener("click",function(event){
    ...
    if(!key){
      if(/^Политика конфиденциальности$/i.test(text))key="privacy";
      else if(/^Согласие на обработку данных$/i.test(text))key="consent";
      else if(/^Оплата$/i.test(text))key="payment";
      else if(/^Доставка$/i.test(text))key="delivery";
      else if(/^Возврат$/i.test(text))key="returns";
      else if(/^Оферта$/i.test(text))key="offer";
    }
...
function addContactRequisites(){
  ...
  var anchor=[].slice.call(main.querySelectorAll("section,div")).find(function(node){
    return /ТЕЛЕФОН|EMAIL|АДРЕС/i.test(node.textContent||"");
  })||main.firstElementChild||main;
  var card=document.createElement("section");
  card.className="gp-requisites-card";
  card.innerHTML='<div class="gp-requisites-icon" aria-hidden="true">▤</div><div><p class="gp-requisites-label">Реквизиты</p><h2>ООО \"Гармония Плюс\"</h2>'+gpDetailsList()+'</div>';
...
gpLegalDocuments={
  privacy:{
    ...
    sections:[
      ["Продавец",["ООО \"Гармония Плюс\", ИНН 1901084417, ОГРН 1081901002028.","Адрес: 655017, Республика Хакасия, г. Абакан, ул. Торосово, 9а.","Телефон: 8-913-448-7333, 8-983-191-03-83. Email: Abakan-sib@mail.ru."]],
      ["Персональные данные",["Обрабатываются имя, телефон, email, адрес доставки, состав заказа, комментарии к заказу и технические данные посещения сайта.","Данные используются для оформления заказа, консультации, оплаты, доставки, возврата, направления чеков и связи с покупателем.","Данные платежей обрабатываются платежным сервисом и банками; сайт не хранит полные данные банковских карт."]],
      ["Оплата",["Заказы оплачиваются в рублях РФ.","Доступные способы оплаты: банковская карта онлайн через платежный сервис ЮKassa после подключения онлайн-оплаты, безналичный перевод на расчетный счет или иной способ, согласованный с менеджером.","После оформления заказа менеджер подтверждает наличие товаров, итоговую сумму и доступный способ оплаты. При ошибке оплаты заказ не считается оплаченным."]],
      ["Доставка",["Доставка согласуется с покупателем после оформления заказа и подтверждения наличия товаров.","Доставка возможна по г. Абакану, Республике Хакасия и в другие регионы РФ по согласованию.","Способ доставки, сроки и стоимость зависят от адреса, состава заказа и выбранной службы доставки. Итоговая стоимость сообщается до оплаты или передачи заказа."]],
      ["Возврат",["Возврат и обмен товаров выполняются в соответствии с законодательством РФ.","Возврат товара надлежащего качества возможен, если товар не был в употреблении, сохранены товарный вид, упаковка, потребительские свойства и документы о покупке.","Для отдельных категорий косметической, парфюмерной, санитарно-гигиенической и медицинской продукции возврат товара надлежащего качества может быть ограничен законодательством РФ.","При браке, повреждении или ошибке комплектации покупатель может обратиться по телефону или email; после проверки обращения продавец согласует замену, возврат товара или возврат денежных средств."]],
      ["Оферта",["Информация о товарах, ценах и наличии на сайте является предложением оформить заказ.","Заказ считается принятым после подтверждения продавцом состава, наличия, цены, оплаты и доставки.","Условия оплаты, доставки и возврата, указанные в настоящем документе, являются частью условий продажи товаров через сайт garmoniyaplus.ru."]],
      ["Права пользователя",["Пользователь может запросить уточнение, блокирование или удаление персональных данных, если их хранение не требуется законом.","Обращения принимаются по email: Abakan-sib@mail.ru."]]
    ]
  },
  consent:{
...
function openInstallQr(target){
  ...
  modal.innerHTML='<div class="gp-install-qr-backdrop" data-gp-install-close="1"></div><div class="gp-install-qr-card" role="dialog" aria-modal="true" aria-label="QR-код для скачивания приложения"><button type="button" class="gp-install-qr-close" data-gp-install-close="1" aria-label="Закрыть QR-код">×</button><div class="gp-install-qr-eyebrow">Скачать приложение</div><h2>QR-код для '+title+'</h2><p>Отсканируйте код камерой телефона</p><img src="/icons/install-app-qr-20260612.png" alt="QR-код для установки приложения" loading="eager"></div>';
...
function gpInstallGuideSteps(profile){
  var iosSafari=[
    ["Откройте сайт в Safari","Если вы сейчас в другом приложении, нажмите «Поделиться» и откройте страницу в Safari.","/install-guides/ios-safari-browser.png"],
    ["Нажмите кнопку «Поделиться»","В Safari она находится в нижней панели браузера.","/install-guides/ios-safari-sheet.png"],
    ["Выберите «На экран Домой»","Прокрутите список действий, если пункт не виден сразу.","/install-guides/ios-safari-sheet.png"],
    ["Нажмите «Добавить»","После подтверждения иконка «ГармонияПлюс» появится на экране телефона.","/install-guides/ios-add-confirm.png"],
    ["Откройте приложение с экрана телефона","Запускайте сайт через новую иконку, как обычное приложение.","/install-guides/ios-home.png"]
  ];
  var iosChrome=[
    ["Откройте сайт в Chrome на iPhone","Убедитесь, что открыта страница garmoniyaplus.ru.","/install-guides/ios-chrome-share.png"],
    ["Нажмите «Поделиться»","Кнопка находится в панели Chrome.","/install-guides/ios-chrome-share.png"],
    ["Откройте дополнительные действия","Если пункта установки нет сразу, нажмите «Ещё».","/install-guides/ios-chrome-more.png"],
    ["Выберите добавление на главный экран","Подтвердите добавление приложения.","/install-guides/ios-chrome-confirm.png"],
    ["Откройте иконку «ГармонияПлюс»","Иконка появится на главном экране iPhone.","/install-guides/ios-home.png"]
  ];
  var androidChrome=[
    ["Откройте сайт в Chrome","Проверьте, что открыт garmoniyaplus.ru, затем нажмите меню с тремя точками.","/install-guides/android-chrome-browser.png"],
    ["Выберите «Добавить на главный экран»","В меню Chrome нажмите пункт установки или добавления на экран.","/install-guides/android-chrome-menu.png"],
    ["Подтвердите установку","Если появится выбор, нажмите «Установить» или «Создать ярлык».","/install-guides/android-chrome-dialog.png"],
    ["Откройте приложение с рабочего стола","Иконка появится на экране телефона среди приложений."]
  ];
  var androidYandex=[
    ["Откройте сайт в Яндекс Браузере","Нажмите меню браузера рядом со строкой адреса.","/install-guides/android-yandex-browser.png"],
    ["Выберите добавление ярлыка","Нажмите «Добавить ярлык на рабочий стол» или похожий пункт.","/install-guides/android-yandex-menu.png"],
    ["Подтвердите создание ярлыка","Если телефон спросит подтверждение, нажмите «Добавить» или «Создать».","/install-guides/android-yandex-menu.png"],
    ["Откройте «ГармонияПлюс» с рабочего стола","После добавления используйте новую иконку как приложение."]
  ];
  if(profile==="ios-safari")return{title:"Инструкция для iPhone в Safari",steps:iosSafari};
  if(profile==="ios-chrome")return{title:"Инструкция для iPhone в Chrome",steps:iosChrome};
  if(profile==="ios-other")return{title:"Инструкция для iPhone",steps:iosSafari,extra:"Если вы открыли сайт в приложении Яндекс с Алисой или внутри мессенджера, сначала откройте страницу в Safari или Chrome."};
  if(profile==="android-yandex")return{title:"Инструкция для Android в Яндекс Браузере",steps:androidYandex};
  if(profile==="android-chrome")return{title:"Инструкция для Android в Chrome",steps:androidChrome};
  return{title:"Инструкция для Android",steps:androidChrome,extra:"Если у вас Яндекс Браузер, используйте пункт «Добавить ярлык на рабочий стол» в меню браузера."};
}
...
function replaceContactsText(){
  ...
  [
    ...
    ["Москва, Цветной бульвар, 21с3","655017, Республика Хакасия, г. Абакан, ул. Торосово, 9а"]
...
function gpNormalizeVisibleProductPrices(){
  ...
  scopes.forEach(function(scope){
    [].slice.call(scope.querySelectorAll("*")).forEach(function(element){
      ...
      if(!next||!/(₽|руб\.?|в‚Ѕ)/i.test(next.textContent||""))return;
    ...
    function walk(node){
      if(!node)return;
      if(node.nodeType===3){
        if(!/(₽|руб\.?|в‚Ѕ)/i.test(node.nodeValue||""))return;
...
function gpIsProductTabLabel82(value){
  ...
  return text==="Описание"||text==="Характеристики";
}
...
function gpFindProductTabSection82(fromButton){
  ...
  for(var index=0;index<sections.length;index+=1){
    ...
    var hasDescription=buttons.some(function(button){return gpCleanText82(button.textContent)==="Описание"});
    var hasSpecs=buttons.some(function(button){return gpCleanText82(button.textContent)==="Характеристики"});
...
function gpFindProductTabContent82(section){
  ...
  return [].slice.call(section.children||[]).find(function(child){
    ...
    return /Описание и активы|Активные компоненты|Артикул|Формат|Категория/i.test(gpCleanText82(child.textContent));
  })||null;
}
...
function gpProductOriginalDescriptionHtml82(section,content){
  ...
  if(!gpProductDescriptionCache82[key]&&content&&/Описание и активы|Активные компоненты/i.test(gpCleanText82(content.textContent))){
...
function gpExtractProductPrice82(main,pageText){
  var candidates=[].slice.call((main||document).querySelectorAll("*")).filter(function(node){
    ...
    return /^\d[\d\s]{0,10}\s*(?:₽|руб\.?|Р)$/i.test(text);
  }).map(function(text){
  ...
  var priceMatch=(pageText||"").match(/\b\d{1,3}(?:\s\d{3})*\s*(?:₽|руб\.?|Р\b)/i);
...
function gpExtractProductSpecs82(section,content){
  ...
  var title=titleNode?gpCleanText82(titleNode.textContent):gpCleanText82(document.title).replace(/^ГармонияПлюс\s*/i,"");
  ...
  var activeNode=[].slice.call(temp.querySelectorAll("*")).find(function(node){return /^Активные компоненты/i.test(gpCleanText82(node.textContent))});
  var active=activeNode?gpCleanText82(activeNode.textContent).replace(/^Активные компоненты\s*/i,""):"";
  var backLink=[].slice.call(main.querySelectorAll("a,button")).find(function(node){return /Назад в:/i.test(gpCleanText82(node.textContent))});
  var category=backLink?gpCleanText82(backLink.textContent).replace(/^←?\s*Назад в:\s*/i,""):"";
  ...
  var formatMatch=title.match(/\d+(?:[,.]\d+)?\s*(?:мл|ml|гр|г|кг|шт|ампул|мг)\b/i);
...
function gpRenderProductSpecs82(section,content){
  ...
  var rows=[
    ["Артикул",specs.sku],
    ["Категория / серия",specs.category],
    ["Формат",specs.format],
    ["Цена",specs.price],
    ["Название",specs.title],
    ["Описание",specs.description],
    ["Активные компоненты",specs.active]
  ].map(function(row){return gpProductDetailValue82(row[0],row[1])}).join("");
  content.className="gp-product-specs82";
  content.innerHTML=rows||'<article class="gp-product-spec-card82"><div class="gp-product-spec-label82">Характеристики</div><div class="gp-product-spec-value82">Информация уточняется.</div></article>';
}
...
function ensureMobileHeader83Style(){
  ...
  style.textContent=[
    ...
    "@media (max-width:1023px){[data-mobile-shell='header'] a[aria-label],[data-mobile-shell='header'] button[aria-label]{min-width:3.05rem!important;align-items:center!important;justify-content:flex-start!important;gap:.1875rem!important;padding:.125rem .125rem!important}}",
    ...
    "@media (max-width:1023px){[data-mobile-shell='header'] button[aria-label='Поиск']{min-width:3.05rem!important}[data-mobile-shell='header'] button[aria-label='Поиск'] svg{width:20px!important;height:20px!important;min-width:20px!important;min-height:20px!important;scale:1!important}[data-mobile-shell='header'] button[aria-label='Поиск'] span[class*='text-[9px]']{font-size:9px!important;line-height:9px!important;height:9px!important}}"
  ].join("");
  document.head.appendChild(style);
}
...
function normalizeMobileHeader83(){
  ...
  actionNodes.forEach(function(action){
    action.style.setProperty("min-width","3.05rem","important");
    action.style.setProperty("align-items","center","important");
    action.style.setProperty("justify-content","flex-start","important");
    action.style.setProperty("gap",".1875rem","important");
    action.style.setProperty("padding",".125rem .125rem","important");
...
function ensureAndroidFast84Style(){
  ...
  style.textContent=[
    "@media (max-width:640px){body.gp-pravki-current-account [data-mobile-shell='header']{height:72px!important;min-height:0!important;max-height:72px!important;border-bottom:1px solid rgba(255,255,255,.94)!important;box-shadow:none!important;overflow:hidden!important}}",
    "@media (max-width:640px){body.gp-pravki-current-account [data-mobile-shell='header']>div:first-child{padding-bottom:0!important}}",
    "@media (max-width:640px){body.gp-pravki-current-account{padding-top:72px!important}}",
...
function normalizeAndroidAccountHeader84(){
  ...
  if(header){
    ...
    header.style.setProperty("max-height","72px","important");
  ...
  document.body.style.setProperty("padding-top","72px","important");
...
function installProductTabsFix82(){
  ...
  if(section&&content){
    ...
    [].slice.call(section.querySelectorAll("button")).forEach(function(button){
      ...
      if(!button.hasAttribute("aria-selected"))button.setAttribute("aria-selected",gpCleanText82(button.textContent)==="Описание"?"true":"false");
    });
  }
...
if("serviceWorker"in navigator){
  var update=function(){
    navigator.serviceWorker.register("/service-worker.js?v26-android-fast84-20260618",{updateViaCache:"none"}).then(function(worker){
