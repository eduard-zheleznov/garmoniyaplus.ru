...
var FULL_RUNTIME_SRC="/_next/static/chunks/gp-runtime-full95.js?v=20260729-product-contacts-docs-v4";
...
function ensureStyle(){
  ...
  style.textContent=[
    ...
    "html.gp-favorites-route93 [data-gp-lite-favorites-page] .gp-lite-favorites-actions{display:flex!important;align-items:flex-end!important;justify-content:flex-end!important;gap:.55rem!important;margin:0!important;position:relative!important;z-index:12!important;pointer-events:auto!important}.gp-lite-favorites-action{position:relative!important;z-index:13!important;display:grid!important;min-width:2.55rem!important;justify-items:center!important;gap:.3rem!important;border:0!important;background:transparent!important;box-shadow:none!important;padding:0!important;color:var(--brand-text,#241d19)!important;pointer-events:auto!important;cursor:pointer!important}.gp-lite-favorites-action *{pointer-events:none!important}.gp-lite-favorites-action-bubble{width:2.45rem!important;height:2.45rem!important}.gp-lite-favorites-heart{left:auto!important;right:1rem!important;top:1rem!important;bottom:auto!important;z-index:8!important}.gp-lite-favorites-heart~.gp-lite-favorites-heart{display:none!important}",
...
function gpLiteRenderFavorites93(){
  ...
  gpLiteRefreshFavoriteActions139(existing);
}
...
function gpLiteClosestAction139(target,selector,event){
  var path=[];
  try{path=event&&event.composedPath?event.composedPath():[]}catch(pathError){path=[]}
  for(var index=0;index<path.length;index++){
    var item=path[index];
    try{if(item&&item.matches&&item.matches(selector))return item}catch(matchError){}
  }
  var node=target;
  if(node&&node.nodeType!==1)node=node.parentElement||node.parentNode;
  while(node&&node!==document&&node!==window){
    try{if(node.matches&&node.matches(selector))return node}catch(error){}
    try{if(node.closest){var found=node.closest(selector);if(found)return found}}catch(closestError){}
...
function gpLiteMarkEventHandled139(event){
  if(!event)return false;
  if(event.__gpLiteActionHandled139)return true;
  try{event.__gpLiteActionHandled139=true}catch(error){}
  return false;
}

function gpLiteStopActionEvent139(event){
  if(!event)return;
  try{event.preventDefault()}catch(preventError){}
  try{event.stopPropagation()}catch(stopError){}
  try{if(event.stopImmediatePropagation)event.stopImmediatePropagation()}catch(immediateError){}
}

function gpLiteRunFavoriteAction139(button,kind,event){
  if(!button||!kind)return false;
  if(gpLiteMarkEventHandled139(event))return true;
  gpLiteStopActionEvent139(event);
  if(kind==="favorite"){
    var favoriteId=gpLiteCanonicalId93(button.getAttribute("data-gp-compare-favorite"));
    if(!favoriteId)return true;
    gpLiteRemoveFavoriteFast109(button,favoriteId);
    return true;
  }
  if(kind==="compare"){
    var compareId=gpLiteCanonicalId93(button.getAttribute("data-gp-compare-toggle"));
    if(!compareId)return true;
    ...
    gpLiteSetActionVisual135(button,"compare",compareActive);
    gpLiteSyncActionState135("compare",compareWritten);
    gpLiteSyncFavoriteActionButtons135(document);
    return true;
  }
  if(kind==="cart"){
    var cartId=gpLiteCanonicalId93(button.getAttribute("data-gp-compare-cart"));
    ...
    gpLiteSetActionVisual135(button,"cart",cartActive);
...
function gpLiteBindFavoriteActionButtons139(root){
  root=root||document;
  [].slice.call(root.querySelectorAll("[data-gp-compare-favorite],[data-gp-compare-toggle],[data-gp-compare-cart]")).forEach(function(button){
    if(button.dataset.gpActionBound139)return;
    button.dataset.gpActionBound139="true";
    button.addEventListener("click",function(event){
      if(!gpRefreshFavoritesRoute93())return;
      if(button.hasAttribute("data-gp-compare-favorite"))gpLiteRunFavoriteAction139(button,"favorite",event);
      else if(button.hasAttribute("data-gp-compare-toggle"))gpLiteRunFavoriteAction139(button,"compare",event);
      else if(button.hasAttribute("data-gp-compare-cart"))gpLiteRunFavoriteAction139(button,"cart",event);
    },true);
  });
}

function gpLiteRefreshFavoriteActions139(root){
  root=root||document;
  gpLiteSyncFavoriteActionButtons135(root);
  gpLiteBindFavoriteActionButtons139(root);
}
...
function gpLiteInstallActionHandlers93(){
  ...
  document.addEventListener("click",function(event){
    if(!gpRefreshFavoritesRoute93())return;
    var favorite=gpLiteClosestAction139(event.target,"[data-gp-compare-favorite]",event);
    if(favorite){
      gpLiteRunFavoriteAction139(favorite,"favorite",event);
      return;
    }
    var compare=gpLiteClosestAction139(event.target,"[data-gp-compare-toggle]",event);
    var cart=gpLiteClosestAction139(event.target,"[data-gp-compare-cart]",event);
    if(compare){
      gpLiteRunFavoriteAction139(compare,"compare",event);
      return;
    }
    if(cart){
      gpLiteRunFavoriteAction139(cart,"cart",event);
      return;
    }
  },true);
}
...
function scheduleLite(){
  ...
  gpLiteRefreshFavoriteActions139();
  gpLiteInstallFavoritesObserver93();
  gpLiteInstallActionHandlers93();
  setTimeout(normalizeHeader,120);
  setTimeout(function(){gpLiteRenderFavorites93();gpLiteRefreshFavoriteActions139()},120);
  setTimeout(normalizeHeader,520);
  setTimeout(function(){gpLiteRenderFavorites93();gpLiteRefreshFavoriteActions139()},520);
  setTimeout(function(){gpLiteRenderFavorites93();gpLiteRefreshFavoriteActions139()},1500);
}
...
function preloadFullRuntime141(){
  if(window.__gpFullRuntime141Preloaded||!document.head)return;
  window.__gpFullRuntime141Preloaded=true;
  try{
    var link=document.createElement("link");
    link.rel="preload";
    link.as="script";
    link.href=FULL_RUNTIME_SRC;
    document.head.appendChild(link);
  }catch(error){}
}

function scheduleFullRuntime(){
  preloadFullRuntime141();
  if(window.requestAnimationFrame)requestAnimationFrame(loadFullRuntime);
  else setTimeout(loadFullRuntime,0);
  setTimeout(loadFullRuntime,isMobile()?700:350);
}

preloadFullRuntime141();
ready(function(){scheduleLite();scheduleFullRuntime()});
addEventListener("pageshow",scheduleLite,{passive:true});
addEventListener("pageshow",scheduleFullRuntime,{passive:true});
...
;(function(){
  var marker="gp-requisites-initial-layout-v20260729";
  var observer=null;
  function isContacts(){return location.pathname.indexOf("/contacts")===0;}
  function set(node,property,value){
    if(!node)return;
    if(node.style.getPropertyValue(property)===String(value)&&node.style.getPropertyPriority(property)==="important")return;
    node.style.setProperty(property,value,"important");
  }
  function ensureStyle(){
    if(!document.head)return null;
    var style=document.getElementById(marker);
    if(!style){
      style=document.createElement("style");
      style.id=marker;
      style.textContent=[
        "body.gp-path-contacts .gp-requisites-card{display:grid!important;grid-template-columns:minmax(0,1fr)!important;width:100%!important;max-width:none!important;margin:1.1rem 0 0!important;padding:1.35rem clamp(1rem,3vw,2rem)!important;box-sizing:border-box!important;justify-items:stretch!important;text-align:center!important}",
        "body.gp-path-contacts .gp-requisites-card>div:not(.gp-requisites-icon){display:block!important;width:100%!important;max-width:none!important;text-align:center!important}",
        "body.gp-path-contacts .gp-requisites-card .gp-requisites-label,body.gp-path-contacts .gp-requisites-card h2{text-align:center!important}",
        "body.gp-path-contacts .gp-requisites-card .gp-legal-details{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:.72rem!important;width:100%!important;max-width:none!important;margin:1rem 0 0!important;text-align:left!important}",
        "body.gp-path-contacts .gp-requisites-card .gp-legal-details>div{display:flex!important;min-width:0!important;min-height:4.35rem!important;align-items:center!important;justify-content:flex-start!important;flex-direction:row!important;gap:1.15rem!important;overflow:hidden!important;border:1px solid rgba(175,90,102,.12)!important;border-radius:1rem!important;background:rgba(255,255,255,.76)!important;padding:.78rem 1.15rem!important;box-sizing:border-box!important;text-align:left!important}",
        "body.gp-path-contacts .gp-requisites-card .gp-legal-details>div:first-child{display:none!important}",
        "body.gp-path-contacts .gp-requisites-card .gp-legal-details span{display:block!important;flex:0 0 3.7rem!important;max-width:3.7rem!important;margin:0!important;color:var(--brand-muted,#8b7b72)!important;font-size:.86rem!important;font-weight:600!important;letter-spacing:.13em!important;line-height:1.2!important;text-align:left!important;text-transform:uppercase!important;white-space:nowrap!important}",
        "body.gp-path-contacts .gp-requisites-card .gp-legal-details strong{display:block!important;flex:0 0 auto!important;width:auto!important;max-width:none!important;min-width:0!important;margin:0!important;overflow-wrap:normal!important;color:var(--brand-text,#241d19)!important;font-size:clamp(1.08rem,1.28vw,1.16rem)!important;font-weight:600!important;line-height:1.2!important;letter-spacing:.01em!important;text-align:left!important;white-space:nowrap!important}",
        "@media(min-width:768px){.gp-path-contacts .gp-requisites-card{position:relative!important;left:50%!important;width:min(calc(100vw - 3rem),84rem)!important;max-width:min(calc(100vw - 3rem),84rem)!important;margin-left:0!important;margin-right:0!important;transform:translateX(-50%)!important}}",
        "@media(max-width:767px){.gp-path-contacts .gp-requisites-card{padding:1.15rem 1rem!important}.gp-path-contacts .gp-requisites-card .gp-legal-details{grid-template-columns:minmax(0,1fr)!important;gap:.54rem!important}.gp-path-contacts .gp-requisites-card .gp-legal-details>div{min-height:4rem!important;padding:.72rem .9rem!important;gap:.9rem!important}.gp-path-contacts .gp-requisites-card .gp-legal-details span{flex-basis:3.35rem!important;max-width:3.35rem!important;font-size:.78rem!important}.gp-path-contacts .gp-requisites-card .gp-legal-details strong{font-size:1.04rem!important}}"
      ].join("");
      document.head.appendChild(style);
    }
  ...
  function applyInline(){
    ...
    var card=document.querySelector("section.gp-requisites-card");
    if(!card)return;
    var desktop=window.matchMedia&&window.matchMedia("(min-width:768px)").matches;
    var details=card.querySelector(".gp-legal-details");
    if(!details)return;
    set(details,"display","grid");
    set(details,"grid-template-columns",desktop?"repeat(2,minmax(0,1fr))":"minmax(0,1fr)");
    set(details,"gap",desktop?".72rem":".54rem");
    set(details,"width","100%");
    set(details,"max-width","none");
    set(details,"margin","1rem 0 0");
    [].slice.call(details.children).forEach(function(row,index){
      if(index===0){set(row,"display","none");return;}
      set(row,"display","flex");
      set(row,"flex-direction","row");
      set(row,"align-items","center");
      set(row,"justify-content","flex-start");
      set(row,"gap",desktop?"1.15rem":".9rem");
      set(row,"min-height",desktop?"4.35rem":"4rem");
      set(row,"padding",desktop?".78rem 1.15rem":".72rem .9rem");
      set(row,"text-align","left");
      var label=row.querySelector("span");
      var value=row.querySelector("strong");
      set(label,"flex",desktop?"0 0 3.7rem":"0 0 3.35rem");
      set(label,"max-width",desktop?"3.7rem":"3.35rem");
      set(label,"margin","0");
      set(label,"text-align","left");
      set(label,"font-size",desktop?".86rem":".78rem");
      set(value,"display","block");
      set(value,"flex","0 0 auto");
      set(value,"width","auto");
      set(value,"max-width","none");
      set(value,"margin","0");
      set(value,"text-align","left");
      set(value,"font-size",desktop?"clamp(1.08rem,1.28vw,1.16rem)":"1.04rem");
      set(value,"font-weight","600");
      set(value,"line-height","1.2");
      set(value,"letter-spacing",".01em");
      set(value,"white-space","nowrap");
    });
  }
  function stabilize(){ensureStyle();applyInline();}
  function watch(){
    stabilize();
    if(observer||!document.documentElement)return;
    observer=new MutationObserver(function(){stabilize();});
...
;(function(){
try{
  var gpLegalRequisitesMarker="gp-legal-requisites-v20260729";
  var gpLegalRequisitesObserver=null;
  function gpSetLegalRequisitesStyle(node,property,value){if(node)node.style.setProperty(property,value,"important");}
  function gpApplyLegalRequisites(){
    [].slice.call(document.querySelectorAll(".gp-legal-dialog .gp-legal-details")).forEach(function(details){
      var compact=window.matchMedia&&window.matchMedia("(max-width:640px)").matches;
      gpSetLegalRequisitesStyle(details,"display","grid");
      gpSetLegalRequisitesStyle(details,"grid-template-columns",compact?"minmax(0,1fr)":"repeat(2,minmax(0,1fr))");
      gpSetLegalRequisitesStyle(details,"gap",compact?".46rem":".52rem .6rem");
      gpSetLegalRequisitesStyle(details,"width","100%");
      gpSetLegalRequisitesStyle(details,"margin",".65rem 0 0");
      [].slice.call(details.children).forEach(function(row){
        gpSetLegalRequisitesStyle(row,"display","flex");
        gpSetLegalRequisitesStyle(row,"flex-direction","row");
        gpSetLegalRequisitesStyle(row,"align-items","center");
        gpSetLegalRequisitesStyle(row,"justify-content","space-between");
        gpSetLegalRequisitesStyle(row,"gap",".6rem");
        gpSetLegalRequisitesStyle(row,"min-width","0");
        gpSetLegalRequisitesStyle(row,"min-height",compact?"2.5rem":"2.7rem");
        gpSetLegalRequisitesStyle(row,"padding",compact?".58rem .68rem":".62rem .72rem");
        gpSetLegalRequisitesStyle(row,"box-sizing","border-box");
        gpSetLegalRequisitesStyle(row,"overflow","hidden");
        gpSetLegalRequisitesStyle(row,"border","1px solid rgba(175,90,102,.14)");
        gpSetLegalRequisitesStyle(row,"border-radius",".9rem");
        gpSetLegalRequisitesStyle(row,"background","rgba(255,255,255,.82)");
        var label=row.querySelector("span");
        var value=row.querySelector("strong");
        gpSetLegalRequisitesStyle(label,"display","block");
        gpSetLegalRequisitesStyle(label,"flex","0 0 auto");
        gpSetLegalRequisitesStyle(label,"margin","0");
        gpSetLegalRequisitesStyle(label,"white-space","nowrap");
        gpSetLegalRequisitesStyle(label,"font-size",compact?".68rem":".7rem");
        gpSetLegalRequisitesStyle(label,"font-weight","600");
        gpSetLegalRequisitesStyle(label,"line-height","1.15");
        gpSetLegalRequisitesStyle(label,"letter-spacing",".06em");
        gpSetLegalRequisitesStyle(label,"text-align","left");
        gpSetLegalRequisitesStyle(value,"display","block");
        gpSetLegalRequisitesStyle(value,"min-width","0");
        gpSetLegalRequisitesStyle(value,"margin","0");
        gpSetLegalRequisitesStyle(value,"white-space","nowrap");
        gpSetLegalRequisitesStyle(value,"overflow","hidden");
        gpSetLegalRequisitesStyle(value,"text-overflow","ellipsis");
        gpSetLegalRequisitesStyle(value,"font-size",compact?".82rem":".88rem");
        gpSetLegalRequisitesStyle(value,"font-weight","600");
        gpSetLegalRequisitesStyle(value,"line-height","1.15");
        gpSetLegalRequisitesStyle(value,"letter-spacing","0");
        gpSetLegalRequisitesStyle(value,"text-align","right");
      });
    });
  }
  function gpWatchLegalRequisites(){
    gpApplyLegalRequisites();
    if(gpLegalRequisitesObserver||!document.body)return;
    gpLegalRequisitesObserver=new MutationObserver(function(mutations){
      if(mutations.some(function(mutation){return mutation.type==="childList"&&mutation.addedNodes&&mutation.addedNodes.length;}))gpApplyLegalRequisites();
    });
    gpLegalRequisitesObserver.observe(document.body,{childList:true,subtree:true});
  }
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",gpWatchLegalRequisites,{once:true});else gpWatchLegalRequisites();
  [250,900,2200,4200].forEach(function(delay){setTimeout(gpApplyLegalRequisites,delay);});
  window.addEventListener("pageshow",gpWatchLegalRequisites,{passive:true});
}catch(error){console.warn("gp legal requisites fix failed",error);}
})();
;(function(){
try{
  var gpRequisitesInlineStyleMarker="gp-requisites-inline-size-v20260729";
  function gpSetRequisitesInlineStyle(node,property,value){if(node)node.style.setProperty(property,value,"important");}
  function gpApplyRequisitesInlineSize(){
    ...
    if(!details)return;
    var desktop=window.matchMedia&&window.matchMedia("(min-width:768px)").matches;
    gpSetRequisitesInlineStyle(details,"display","grid");
    gpSetRequisitesInlineStyle(details,"grid-template-columns",desktop?"repeat(2,minmax(0,1fr))":"minmax(0,1fr)");
    gpSetRequisitesInlineStyle(details,"gap",desktop?".72rem":".54rem");
    gpSetRequisitesInlineStyle(details,"width","100%");
    gpSetRequisitesInlineStyle(details,"margin",".9rem 0 0");
    [].slice.call(details.children).forEach(function(row,index){
      if(index===0){gpSetRequisitesInlineStyle(row,"display","none");return;}
      gpSetRequisitesInlineStyle(row,"display","flex");
      gpSetRequisitesInlineStyle(row,"flex-direction","row");
      gpSetRequisitesInlineStyle(row,"align-items","center");
      gpSetRequisitesInlineStyle(row,"justify-content","flex-start");
      gpSetRequisitesInlineStyle(row,"gap","1.15rem");
      gpSetRequisitesInlineStyle(row,"min-width","0");
      gpSetRequisitesInlineStyle(row,"min-height",desktop?"4.35rem":"4rem");
      gpSetRequisitesInlineStyle(row,"padding",desktop?".78rem 1.15rem":".72rem .9rem");
      gpSetRequisitesInlineStyle(row,"text-align","left");
      var label=row.querySelector("span");
      var value=row.querySelector("strong");
      gpSetRequisitesInlineStyle(label,"display","block");
      gpSetRequisitesInlineStyle(label,"flex","0 0 3.7rem");
      gpSetRequisitesInlineStyle(label,"margin","0");
      gpSetRequisitesInlineStyle(label,"text-align","left");
      gpSetRequisitesInlineStyle(label,"font-size",desktop?".86rem":".78rem");
      gpSetRequisitesInlineStyle(label,"font-weight","600");
      gpSetRequisitesInlineStyle(label,"letter-spacing",".13em");
      gpSetRequisitesInlineStyle(value,"display","block");
      gpSetRequisitesInlineStyle(value,"flex","0 0 auto");
      gpSetRequisitesInlineStyle(value,"width","auto");
      gpSetRequisitesInlineStyle(value,"max-width","none");
      gpSetRequisitesInlineStyle(value,"min-width","0");
      gpSetRequisitesInlineStyle(value,"margin","0");
      gpSetRequisitesInlineStyle(value,"text-align","left");
      gpSetRequisitesInlineStyle(value,"font-size",desktop?"clamp(1.08rem,1.28vw,1.16rem)":"1.04rem");
      gpSetRequisitesInlineStyle(value,"font-weight","600");
      gpSetRequisitesInlineStyle(value,"line-height","1.2");
      gpSetRequisitesInlineStyle(value,"letter-spacing",".01em");
    });
  }
  function gpScheduleRequisitesInlineSize(){
    gpApplyRequisitesInlineSize();
    [180,700,1600,3500,7000,11500,15500,20500,23500].forEach(function(delay){setTimeout(gpApplyRequisitesInlineSize,delay);});
  }
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",gpScheduleRequisitesInlineSize,{once:true});else gpScheduleRequisitesInlineSize();
  window.addEventListener("pageshow",gpScheduleRequisitesInlineSize,{passive:true});
}catch(error){console.warn("gp requisites inline size fix failed",error);}
})();
;(function(){
try{
  var gpProductDescriptionCardStyleId="gp-product-description-card-v20260729-v14";
  function gpProductDescriptionText(node){return (node&&node.textContent||"").replace(/\s+/g," ").trim();}
  function gpIsDescriptionHeading(node){
    return /^(?:\u041e\u043f\u0438\u0441\u0430\u043d\u0438\u0435|Description)$/i.test(gpProductDescriptionText(node));
  }
  function gpIsActiveComponentsHeading(node){
    return /^\u0410\u043a\u0442\u0438\u0432\u043d\u044b\u0435\s+\u043a\u043e\u043c\u043f\u043e\u043d\u0435\u043d\u0442\u044b$/i.test(gpProductDescriptionText(node));
  }
  function gpEnsureProductDescriptionCardStyle(){
    if(!document.head)return;
    var style=document.getElementById(gpProductDescriptionCardStyleId);
    if(!style){
      style=document.createElement("style");
      style.id=gpProductDescriptionCardStyleId;
      style.textContent=[
        ".gp-product-description-shell{display:grid!important;grid-template-columns:minmax(0,1fr)!important;gap:0!important;margin-top:1.5rem!important}",
        ".gp-product-description-card{grid-column:1 / -1!important;width:100%!important;max-width:none!important;box-sizing:border-box!important;background:#fff!important;border:1px solid rgba(255,255,255,.86)!important;border-radius:2rem!important;box-shadow:0 18px 48px rgba(36,29,25,.09)!important;padding:clamp(1.35rem,2.6vw,2.25rem)!important}",
        ".gp-product-description-card>h2{margin:0!important;text-transform:uppercase!important;letter-spacing:.12em!important;font-size:clamp(1.08rem,2vw,1.42rem)!important;line-height:1.12!important;color:var(--brand-text,#241d19)!important}",
        ".gp-product-description-card>p{max-width:none!important;margin:1rem 0 0!important;font-size:1rem!important;line-height:1.7!important;color:var(--brand-muted,#8b7b72)!important}",
        "@media(max-width:640px){.gp-product-description-shell{margin-top:1rem!important}.gp-product-description-card{border-radius:1.45rem!important;padding:1.2rem!important}.gp-product-description-card>p{font-size:.94rem!important;line-height:1.62!important}}"
      ].join("");
      document.head.appendChild(style);
    }else if(style.parentNode){style.parentNode.appendChild(style);}
  }
  function gpApplyProductDescriptionCard(){
    if(location.pathname.indexOf("/product/")!==0)return;
    gpEnsureProductDescriptionCardStyle();
    var main=document.querySelector("main");
    if(!main)return;
    var descriptionArticle=null;
    [].slice.call(main.querySelectorAll("article")).forEach(function(article){
      var heading=article.querySelector("h2");
      if(heading&&gpIsDescriptionHeading(heading))descriptionArticle=article;
      var firstHeading=article.querySelector("h2,h3");
      if(firstHeading&&gpIsActiveComponentsHeading(firstHeading))article.remove();
    });
    var descriptionText=gpProductDescriptionText(descriptionArticle&&descriptionArticle.querySelector("p"));
    var productHeroes=[];
    [].slice.call(main.querySelectorAll("h1")).forEach(function(productHeading){
      var productHero=productHeading.closest("section");
      if(productHero&&productHeroes.indexOf(productHero)===-1)productHeroes.push(productHero);
    });
    productHeroes.forEach(function(productHero){
      [].slice.call(productHero.querySelectorAll("p")).forEach(function(paragraph){
        var value=gpProductDescriptionText(paragraph);
        if(!value||(descriptionText&&value===descriptionText))paragraph.remove();
      });
    });
    [].slice.call(main.querySelectorAll("button")).forEach(function(button){
      var value=gpProductDescriptionText(button);
      if(!/^(?:\u041e\u043f\u0438\u0441\u0430\u043d\u0438\u0435|\u0425\u0430\u0440\u0430\u043a\u0442\u0435\u0440\u0438\u0441\u0442\u0438\u043a\u0438|Description|Characteristics)$/i.test(value))return;
      var group=button.parentElement;
      if(group&&[].slice.call(group.children||[]).every(function(node){return node.tagName==="BUTTON";}))group.remove();
      else button.remove();
    });
    if(!descriptionArticle)return;
    if(!descriptionText){
      var emptyShell=descriptionArticle.closest(".mt-5");
      descriptionArticle.remove();
      if(emptyShell&&!gpProductDescriptionText(emptyShell))emptyShell.remove();
      return;
    }
    var heading=descriptionArticle.querySelector("h2");
    if(heading)heading.textContent="\u041e\u043f\u0438\u0441\u0430\u043d\u0438\u0435";
    [].slice.call(descriptionArticle.querySelectorAll("ul,ol")).forEach(function(node){node.remove();});
    if(descriptionArticle.className!=="gp-product-description-card")descriptionArticle.className="gp-product-description-card";
    var shell=descriptionArticle.closest(".mt-5");
    if(shell)shell.classList.add("gp-product-description-shell");
  }
  function gpScheduleProductDescriptionCard(){
    gpApplyProductDescriptionCard();
    [0,420,1200,2400,3800,5000].forEach(function(delay){setTimeout(gpApplyProductDescriptionCard,delay);});
  }
  gpScheduleProductDescriptionCard();
}catch(error){console.warn("gp product description card fix failed",error);}
})();
;(function(){
  var marker="gp-current-consultant-menu-v20260729";
  function apply(){
    return;
    if(location.pathname.indexOf("/contacts")!==0)return;
    var params=new URLSearchParams(location.search||"");
    if(params.get("consultant")!=="current"&&params.get("chat")!=="consultant")return;
    var root=document.querySelector(".gp-contact-widget-fallback");
    if(!root)return;
    root.setAttribute("data-gp-current-consultant",marker);
    root.classList.remove("is-chat");
    root.style.removeProperty("display");
    var trigger=root.querySelector(".gp-contact-widget-main");
    if(trigger)trigger.setAttribute("aria-expanded","true");
    if(!document.getElementById(marker)){
      var style=document.createElement("style");
      style.id=marker;
      style.textContent=".gp-contact-widget-fallback[data-gp-current-consultant=\""+marker+"\"] .gp-contact-widget-old-menu{display:grid!important}.gp-contact-widget-fallback[data-gp-current-consultant=\""+marker+"\"] .gp-contact-widget-panel{display:none!important}";
      (document.head||document.documentElement).appendChild(style);
    }
  }
  [0,180,700,1700,3200,5000,7000].forEach(function(delay){setTimeout(apply,delay)});
...
function addStyle(){
  ...
  style.textContent=[
    ...
    "body.gp-path-cart.gp-cart-native-ready140 [data-gp-cart-fallback136]{display:none!important;visibility:hidden!important}",
    ...
    ".gp-search-fallback-panel{position:fixed!important;z-index:2147483000!important;max-height:min(22rem,59vh)!important;overflow:auto!important;border:1px solid rgba(175,90,102,.16)!important;border-radius:1.1rem!important;background:rgba(255,255,255,.98)!important;box-shadow:0 20px 56px rgba(36,29,25,.16)!important;padding:.35rem!important;color:var(--brand-text,#241d19)!important}.gp-search-fallback-item{display:flex!important;align-items:center!important;gap:.5rem!important;width:100%!important;border:0!important;border-radius:.78rem!important;background:transparent!important;color:inherit!important;padding:.56rem .7rem!important;text-align:left!important;text-decoration:none!important;font-size:.92rem!important;line-height:1.2!important}.gp-search-fallback-item:hover,.gp-search-fallback-item:focus{background:rgba(175,90,102,.08)!important;outline:0!important}.gp-search-fallback-kind{flex:0 0 auto!important;border-radius:999px!important;background:rgba(175,90,102,.1)!important;color:var(--brand-primary,#af5a66)!important;padding:.18rem .46rem!important;font-size:.65rem!important;line-height:1!important;text-transform:uppercase!important;letter-spacing:.08em!important}.gp-search-fallback-title{min-width:0!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important}.gp-search-fallback-empty{padding:.72rem .82rem!important;color:var(--brand-muted,#8b7b72)!important;font-size:.9rem!important}",
    ...
    "@media(max-width:1023px){html.gp-mobile-filter-open124 body .gp-old-filter-overlay,body.gp-mobile-filter-open124 .gp-old-filter-overlay{display:block!important;background:transparent!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;padding:calc(var(--mobile-header-height,58px) + .65rem + env(safe-area-inset-top)) .55rem calc(var(--mobile-viewport-bottom,0px) + env(safe-area-inset-bottom) + .55rem)!important}html.gp-mobile-filter-open124 body .gp-old-filter-backdrop,body.gp-mobile-filter-open124 .gp-old-filter-backdrop{display:block!important;background:transparent!important}html.gp-mobile-filter-open124 body .gp-old-filter-mobile-shell,body.gp-mobile-filter-open124 .gp-old-filter-mobile-shell{position:relative!important;z-index:1!important;display:block!important;width:100%!important;height:100%!important;min-height:0!important;overflow:hidden!important}html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile{display:block!important;width:100%!important;height:100%!important;min-height:0!important;max-height:none!important;overflow:hidden!important;background:linear-gradient(180deg,rgba(249,245,239,.99),rgba(247,241,234,.97))!important;border:1px solid rgba(255,255,255,.78)!important;border-radius:1.75rem!important;box-shadow:0 20px 54px rgba(36,29,25,.13)!important;padding:0!important;margin:0!important;box-sizing:border-box!important}html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile .gp-old-filter-scroll,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile .gp-old-filter-scroll{height:100%!important;max-height:none!important;min-height:0!important;overflow-y:auto!important;-webkit-overflow-scrolling:touch!important;background:transparent!important;border-radius:inherit!important;padding:1.15rem 1rem calc(1.5rem + env(safe-area-inset-bottom))!important;box-sizing:border-box!important}html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile .gp-old-filter-head,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile .gp-old-filter-head{position:sticky!important;top:-1.15rem!important;z-index:10!important;width:calc(100% + 2rem)!important;min-width:0!important;max-width:none!important;box-sizing:border-box!important;margin:-1.15rem -1rem 1rem!important;padding:1.15rem 1rem 1rem!important;background:rgba(249,245,239,.98)!important;border-bottom:1px solid rgba(175,90,102,.12)!important;box-shadow:0 10px 24px rgba(36,29,25,.06)!important}}",
    "@media(max-width:1023px){html.gp-mobile-filter-open124 body .gp-old-filter-overlay,body.gp-mobile-filter-open124 .gp-old-filter-overlay{position:fixed!important;inset:calc(var(--mobile-header-height,58px) + env(safe-area-inset-top)) 0 calc(var(--mobile-viewport-bottom,0px) + env(safe-area-inset-bottom))!important;display:block!important;width:auto!important;height:auto!important;min-height:0!important;box-sizing:border-box!important;background:transparent!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;padding:0 .55rem .55rem!important}html.gp-mobile-filter-open124 body .gp-old-filter-mobile-shell,body.gp-mobile-filter-open124 .gp-old-filter-mobile-shell{position:relative!important;z-index:1!important;display:block!important;width:100%!important;height:100%!important;min-height:0!important;overflow:hidden!important}html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile{display:block!important;width:100%!important;height:100%!important;min-height:0!important;max-height:none!important;overflow:hidden!important;background:linear-gradient(180deg,rgba(249,245,239,.99),rgba(247,241,234,.97))!important;border:1px solid rgba(175,90,102,.11)!important;border-radius:1.75rem!important;box-shadow:0 14px 38px rgba(36,29,25,.1)!important;padding:0!important;margin:0!important;box-sizing:border-box!important}html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile .gp-old-filter-scroll,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile .gp-old-filter-scroll{height:100%!important;max-height:none!important;min-height:0!important;overflow-y:auto!important;-webkit-overflow-scrolling:touch!important;background:transparent!important;border-radius:inherit!important;padding:1.15rem 1rem calc(1.5rem + env(safe-area-inset-bottom))!important;box-sizing:border-box!important}}",
    "@media(max-width:1023px){html body .gp-old-filter-overlay{position:fixed!important;inset:calc(var(--mobile-header-height,58px) + env(safe-area-inset-top)) 0 calc(var(--mobile-viewport-bottom,0px) + env(safe-area-inset-bottom))!important;display:block!important;width:auto!important;height:auto!important;min-height:0!important;box-sizing:border-box!important;background:transparent!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;padding:0 .55rem .55rem!important}html body .gp-old-filter-mobile-shell{position:relative!important;z-index:1!important;display:block!important;width:100%!important;height:100%!important;min-height:0!important;overflow:hidden!important}html body .gp-old-filter-panel.is-mobile{display:block!important;width:100%!important;height:100%!important;min-height:0!important;max-height:none!important;overflow:hidden!important;background:linear-gradient(180deg,rgba(249,245,239,.99),rgba(247,241,234,.97))!important;border:1px solid rgba(175,90,102,.11)!important;border-radius:1.75rem!important;box-shadow:0 14px 38px rgba(36,29,25,.1)!important;padding:0!important;margin:0!important;box-sizing:border-box!important}html body .gp-old-filter-panel.is-mobile .gp-old-filter-scroll{height:100%!important;max-height:none!important;min-height:0!important;overflow-y:auto!important;-webkit-overflow-scrolling:touch!important;background:transparent!important;border-radius:inherit!important;padding:1.15rem 1rem calc(1.5rem + env(safe-area-inset-bottom))!important;box-sizing:border-box!important}}",
    ".gp-contact-widget-icon-bubble-v2.gp-vk-brand-icon,.gp-contact-widget-old-card .gp-contact-widget-icon-bubble-v2.gp-vk-brand-icon,.gp-contact-widget-main .gp-contact-widget-icon-bubble-v2.gp-vk-brand-icon{background:#0077ff!important;color:#fff!important}.gp-contact-widget-icon-bubble-v2.gp-vk-brand-icon svg,.gp-contact-widget-old-card .gp-contact-widget-icon-bubble-v2.gp-vk-brand-icon svg,.gp-contact-widget-main .gp-contact-widget-icon-bubble-v2.gp-vk-brand-icon svg{display:block!important;width:1.25rem!important;height:1.25rem!important;color:#fff!important;fill:currentColor!important}"
  ].join("");
  document.head.appendChild(style);
}
...
function installMobileFilterCloseSync124(){
  if(window.__gpMobileFilterCloseSync124)return;
  window.__gpMobileFilterCloseSync124=true;
  document.addEventListener("click",function(event){
    var control=event.target&&event.target.closest&&event.target.closest("[data-gp-close-filters],.gp-old-filter-close");
    if(!control)return;
    setTimeout(markMobileFilterOpen124,0);
  },true);
}
...
function mergeSearchStaticIndex123(items){
  ...
  items.forEach(function(item){
    ...
    if(href.indexOf("/product/")===0){
      searchAddProduct123(products,item.title,href,[item.category,item.brand,item.subtitle,item.section].join(" "));
      return;
    }
    if(href.indexOf("/catalog")===0)searchAddCategory123(categories,item.title,href);
  });
  window.__gpSearchProducts101=products;
  window.__gpSearchCategories123=categories;
  return true;
...
function ensureSearchIndex123(input,query){
  ...
  var loadCatalogFallback=function(){
    if(typeof DOMParser==="undefined")return Promise.resolve();
    return Promise.all(GP_SEARCH_INDEX_URLS123.map(function(url){
  ...
  fetch("/search-index.v20260604-docx5-visible.json",{credentials:"same-origin",cache:"force-cache"}).then(function(response){
    return response&&response.ok?response.json():null;
  }).then(function(items){
    if(mergeSearchStaticIndex123(items))return;
...
function buildSearchItems(query){
  ...
  var categoryMatches=matches.filter(function(item){return String(item.href||"").indexOf("/product/")!==0}).sort(function(left,right){
    if(left.score!==right.score)return left.score-right.score;
    if(String(left.title).length!==String(right.title).length)return String(left.title).length-String(right.title).length;
    return String(left.title).localeCompare(String(right.title),"ru");
  }).slice(0,1);
  var productMatches=matches.filter(function(item){return String(item.href||"").indexOf("/product/")===0}).slice(0,6);
  items=categoryMatches.concat(productMatches).map(function(item){return{title:item.title,href:item.href,kind:item.kind}});
...
function installSearchFallback(){
  ...
  ensureSearchIndex123(null,"preload");
}
...
function cartOutsideFallbackText140(main){
  if(!main)return "";
  var parts=[];
  [].slice.call(main.querySelectorAll("section,article,aside,div")).forEach(function(node){
    if(node.closest&&node.closest("[data-gp-cart-fallback136]"))return;
...
function hasNativeCartCheckout140(main){
  if(!main)return false;
  var text=cartOutsideFallbackText140(main);
  if(!text)return false;
  var hasCheckoutText=/Перейти\s+к\s+оформлению|Оформить\s+заказ/i.test(text);
  var hasCartTitle=/\d+\s+товар|Корзина|Итого|Удалить/i.test(text);
  if(!hasCheckoutText||!hasCartTitle)return false;
  return [].slice.call(main.querySelectorAll("a[href*='/checkout'],button")).some(function(node){
    if(node.closest&&node.closest("[data-gp-cart-fallback136]"))return false;
    var label=(node.textContent||node.getAttribute("aria-label")||node.getAttribute("title")||"").replace(/\s+/g," ").trim();
    var href=node.getAttribute&&node.getAttribute("href")||"";
    return /Перейти\s+к\s+оформлению|Оформить\s+заказ/i.test(label)||href.indexOf("/checkout")>-1;
  });
}

function removeDuplicateCartFallback140(main){
  if(!main)main=document.querySelector("main");
  if(!main||!hasNativeCartCheckout140(main)){
    if(document.body)document.body.classList.remove("gp-cart-native-ready140");
    return false;
  }
  [].slice.call(main.querySelectorAll("[data-gp-cart-fallback136]")).forEach(function(node){
    if(node.parentNode)node.parentNode.removeChild(node);
  });
  if(document.body)document.body.classList.add("gp-cart-native-ready140","gp-cart-has-local-items");
  return true;
}
...
function renderCartFallback136(){
  ...
  var main=document.querySelector("main");
  if(!main)return;
  if(removeDuplicateCartFallback140(main)){
...
function prioritizeVisibleImages141(root){
  var now=Date.now();
  if(prioritizeVisibleImages141._last&&now-prioritizeVisibleImages141._last<350)return;
  prioritizeVisibleImages141._last=now;
  root=root||document;
  var viewportHeight=window.innerHeight||900;
  var promoted=0;
  [].slice.call(root.querySelectorAll("img")).some(function(img){
    if(!img||img.dataset.gpImagePriority141==="1")return false;
    var rect=img.getBoundingClientRect&&img.getBoundingClientRect();
    if(!rect||rect.width<24||rect.height<24)return false;
    if(rect.top>viewportHeight+320||rect.bottom<-120)return false;
    img.dataset.gpImagePriority141="1";
    img.decoding="async";
    img.loading="eager";
    try{img.fetchPriority=promoted<8?"high":"auto"}catch(error){}
    promoted+=1;
    return promoted>=14;
  });
}
...
function fixCartShell(){
  ...
  if(location.pathname.indexOf("/cart")===0)removeDuplicateCartFallback140();
  hideFalseEmptyCart();
  fixHeaderBadges();
  syncHeaderActionBadges117();
  prioritizeVisibleImages141(document);
}
...
function quickRun(){
  var now=Date.now();
  if(quickRun._last&&now-quickRun._last<180)return;
  quickRun._last=now;
  addPageClasses();
  applyAuthCodeStep();
  ensureFilterIcons();
...
function run(){
  ...
  stabilizeDesktopCatalogGrid131();
  markMobileFilterOpen124();
  installMobileFilterCloseSync124();
...
ready(function(){
  ...
  var schedule=function(){
    if(scheduled)return;
    scheduled=true;
    var delay=Math.max(80,180-(Date.now()-(quickRun._last||0)));
    setTimeout(function(){
      scheduled=false;
      if(window.requestAnimationFrame)requestAnimationFrame(quickRun);
      else quickRun();
    },delay);
  };
  ...
  var mobileInterval=setInterval(function(){
    quickRun();
    tick+=1;
    if(tick>=5)clearInterval(mobileInterval);
  },650);
  setTimeout(function(){try{observer.disconnect()}catch(error){}},5500);
  setTimeout(run,180);
  setTimeout(quickRun,700);
  setTimeout(run,1500);
  setTimeout(quickRun,2800);
});
...
;(function(){
try{
  var gpMobileFilterCleanShellStyleId="gp-mobile-filter-clean-shell-v20260729";
  function gpApplyMobileFilterCleanShell(){
    if(!document.head)return;
    var style=document.getElementById(gpMobileFilterCleanShellStyleId);
    if(!style){
      style=document.createElement("style");
      style.id=gpMobileFilterCleanShellStyleId;
      style.textContent=[
        "@media(max-width:1023px){",
        "html.gp-mobile-filter-open124,html.gp-mobile-filter-open124 body{overflow:hidden!important}",
        "html.gp-mobile-filter-open124 body .gp-old-filter-overlay,body.gp-mobile-filter-open124 .gp-old-filter-overlay{position:fixed!important;inset:0!important;z-index:2147482500!important;display:block!important;width:100%!important;height:100%!important;min-height:100%!important;box-sizing:border-box!important;overflow:hidden!important;background:transparent!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;padding:calc(var(--mobile-header-height,58px) + 1rem + env(safe-area-inset-top)) .55rem calc(var(--mobile-viewport-bottom,0px) + env(safe-area-inset-bottom) + .85rem)!important}",
        "html.gp-mobile-filter-open124 body .gp-old-filter-backdrop,body.gp-mobile-filter-open124 .gp-old-filter-backdrop{display:block!important;background:transparent!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important}",
        "html.gp-mobile-filter-open124 body .gp-old-filter-mobile-shell,body.gp-mobile-filter-open124 .gp-old-filter-mobile-shell{position:relative!important;z-index:1!important;display:block!important;width:100%!important;height:100%!important;min-height:0!important;max-height:none!important;box-sizing:border-box!important;overflow:hidden!important;background:transparent!important}",
        "html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile{position:relative!important;display:block!important;width:100%!important;min-width:0!important;max-width:none!important;height:100%!important;min-height:0!important;max-height:none!important;box-sizing:border-box!important;overflow:hidden!important;margin:0!important;padding:0!important;border:1px solid rgba(255,255,255,.92)!important;border-radius:1.75rem!important;background:#f9f5ef!important;box-shadow:none!important}",
        "html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile .gp-old-filter-scroll,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile .gp-old-filter-scroll{display:block!important;width:100%!important;height:100%!important;min-height:0!important;max-height:none!important;box-sizing:border-box!important;overflow-x:hidden!important;overflow-y:auto!important;-webkit-overflow-scrolling:touch!important;overscroll-behavior:contain!important;background:transparent!important;border-radius:inherit!important;padding:1.15rem 1rem calc(2rem + env(safe-area-inset-bottom))!important}",
        "html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile .gp-old-filter-scroll>*,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile .gp-old-filter-scroll>*{box-sizing:border-box!important;max-width:100%!important}",
        "html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile .gp-old-filter-head,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile .gp-old-filter-head{position:sticky!important;top:-1.15rem!important;z-index:10!important;width:calc(100% + 2rem)!important;min-width:0!important;max-width:none!important;box-sizing:border-box!important;margin:-1.15rem -1rem 1rem!important;padding:1.15rem 1rem 1rem!important;background:#f9f5ef!important;border-bottom:1px solid rgba(175,90,102,.12)!important;box-shadow:none!important}",
        "}"
      ].join("");
      document.head.appendChild(style);
  ...
  gpApplyMobileFilterCleanShell();
  setTimeout(gpApplyMobileFilterCleanShell,0);
  setTimeout(gpApplyMobileFilterCleanShell,700);
}catch(error){console.warn("gp mobile filter clean shell failed",error)}
})();
;(function(){
try{
  var gpMobileFilterNoTopUnderlayStyleId="gp-mobile-filter-no-top-underlay-v20260729";
  function gpApplyMobileFilterNoTopUnderlay(){
    if(!document.head)return;
    var style=document.getElementById(gpMobileFilterNoTopUnderlayStyleId);
    if(!style){
      style=document.createElement("style");
      style.id=gpMobileFilterNoTopUnderlayStyleId;
      style.textContent=[
        "@media(max-width:1023px){",
        "html.gp-mobile-filter-open124 body .gp-old-filter-overlay,body.gp-mobile-filter-open124 .gp-old-filter-overlay{background:linear-gradient(to bottom,transparent 0,transparent calc(var(--mobile-header-height,58px) + env(safe-area-inset-top)),#f9f5ef calc(var(--mobile-header-height,58px) + env(safe-area-inset-top)),#f9f5ef calc(var(--mobile-header-height,58px) + 1rem + env(safe-area-inset-top)),transparent calc(var(--mobile-header-height,58px) + 1rem + env(safe-area-inset-top)))!important}",
        "html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile,html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile .gp-old-filter-scroll,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile .gp-old-filter-scroll,html.gp-mobile-filter-open124 body .gp-old-filter-panel.is-mobile .gp-old-filter-head,body.gp-mobile-filter-open124 .gp-old-filter-panel.is-mobile .gp-old-filter-head{background:#f9f5ef!important}",
        "}"
      ].join("");
      document.head.appendChild(style);
    }else if(style.parentNode){
  ...
  gpApplyMobileFilterNoTopUnderlay();
  setTimeout(gpApplyMobileFilterNoTopUnderlay,760);
  setTimeout(gpApplyMobileFilterNoTopUnderlay,1600);
}catch(error){console.warn("gp mobile filter top underlay cleanup failed",error)}
})();
;(function(){
try{
  var gpProductContactsDocsStyleId="gp-product-contacts-docs-v20260729";
  function gpFinalText(node){return (node&&node.textContent||"").replace(/\s+/g," ").trim();}
  function gpEnsureProductContactsDocsStyle(){
    if(!document.head)return;
    var style=document.getElementById(gpProductContactsDocsStyleId);
    if(!style){
      style=document.createElement("style");
      style.id=gpProductContactsDocsStyleId;
      style.textContent=[
        ".gp-final-product-description{grid-column:1 / -1!important;width:100%!important;max-width:none!important;border:0!important;border-radius:0!important;background:transparent!important;box-shadow:none!important;padding:0!important}",
        ".gp-final-product-description-shell{border:0!important;border-radius:0!important;background:transparent!important;box-shadow:none!important;padding:0!important}",
        ".gp-final-product-description-shell>.mt-5{margin-top:0!important;grid-template-columns:minmax(0,1fr)!important;gap:0!important}",
        ".gp-final-product-description>h2{margin:0!important;text-transform:uppercase!important;letter-spacing:.12em!important;font-size:clamp(1.08rem,2vw,1.42rem)!important;line-height:1.12!important;color:var(--brand-text,#241d19)!important}",
        ".gp-final-product-description>p{max-width:none!important;margin:1rem 0 0!important;font-size:1rem!important;line-height:1.7!important;color:var(--brand-muted,#8b7b72)!important}",
        ".gp-requisites-card{grid-template-columns:1fr!important;justify-items:center!important;text-align:center!important}",
        ".gp-requisites-card .gp-requisites-icon{display:none!important}",
        ".gp-requisites-card>div:not(.gp-requisites-icon){display:grid!important;justify-items:center!important;width:100%!important;text-align:center!important}",
        ".gp-requisites-card .gp-requisites-label{margin:0 0 .45rem!important;text-align:center!important}",
        ".gp-requisites-card h2{margin:0!important;text-align:center!important}",
        ".gp-requisites-card .gp-legal-details{display:grid!important;width:min(100%,26rem)!important;gap:.32rem!important;margin:.8rem auto 0!important;text-align:center!important}",
        ".gp-requisites-card .gp-legal-details>div{display:flex!important;align-items:baseline!important;justify-content:center!important;gap:.48rem!important;min-width:0!important;text-align:center!important}",
        ".gp-requisites-card .gp-legal-details>div:first-child{display:none!important}",
        ".gp-requisites-card .gp-legal-details span,.gp-requisites-card .gp-legal-details strong{min-width:0!important;overflow-wrap:anywhere!important;text-align:center!important}",
        ".gp-legal-dialog .gp-legal-details{display:grid!important;gap:.54rem!important;width:100%!important;max-width:100%!important;text-align:left!important}",
        ".gp-legal-dialog .gp-legal-details>div{display:grid!important;grid-template-columns:minmax(6.25rem,.42fr) minmax(0,1fr)!important;align-items:start!important;gap:.7rem!important;min-width:0!important;text-align:left!important}",
        ".gp-legal-dialog .gp-legal-details span,.gp-legal-dialog .gp-legal-details strong{min-width:0!important;max-width:100%!important;overflow-wrap:anywhere!important;word-break:normal!important;text-align:left!important}",
        "@media(max-width:640px){.gp-legal-dialog .gp-legal-details>div{grid-template-columns:minmax(5.5rem,.42fr) minmax(0,1fr)!important;gap:.52rem!important}.gp-final-product-description>p{font-size:.94rem!important;line-height:1.62!important}}"
      ].join("");
      document.head.appendChild(style);
    }else if(style.parentNode){style.parentNode.appendChild(style);}
  }
  function gpCleanProductDetails(){
    if(location.pathname.indexOf("/product/")!==0)return;
    var main=document.querySelector("main");
    if(!main)return;
    var articles=[].slice.call(main.querySelectorAll("article"));
    var descriptionArticle=null,descriptionText="";
    articles.forEach(function(article){
      var heading=article.querySelector("h2");
      if(!heading||!/^\s*(?:Описание|Description)/i.test(gpFinalText(heading)))return;
      descriptionArticle=article;
      var paragraph=article.querySelector("p");
      descriptionText=gpFinalText(paragraph);
    });
    [].slice.call(main.querySelectorAll("button")).forEach(function(button){
      var value=gpFinalText(button);
      if(value!=="Описание"&&value!=="Характеристики")return;
      var group=button.parentElement;
      if(group&&[].slice.call(group.children||[]).every(function(node){return node.tagName==="BUTTON";}))group.remove();
      else button.remove();
    });
    [].slice.call(main.querySelectorAll("article")).forEach(function(article){
      ...
      if(/^\s*Активные компоненты\s*$/i.test(gpFinalText(first)))article.remove();
    });
    [].slice.call(main.querySelectorAll("h1")).forEach(function(heading){
      var parent=heading.parentElement;
      if(!parent)return;
      [].slice.call(parent.children||[]).forEach(function(child){
        if(child.tagName!=="P")return;
        var value=gpFinalText(child);
        if(!value||!descriptionText||value===descriptionText)child.remove();
      });
    });
    if(!descriptionArticle)return;
    if(!descriptionText){
      var emptyShell=descriptionArticle.closest("section");
      descriptionArticle.remove();
      if(emptyShell&&!gpFinalText(emptyShell))emptyShell.remove();
      return;
    }
    var heading=descriptionArticle.querySelector("h2");
    if(heading)heading.textContent="Описание";
    [].slice.call(descriptionArticle.querySelectorAll("ul,ol")).forEach(function(node){node.remove();});
    descriptionArticle.className="gp-final-product-description";
    var descriptionShell=descriptionArticle.closest("section");
    if(descriptionShell&&descriptionShell.children.length===1)descriptionShell.classList.add("gp-final-product-description-shell");
  }
  function gpRefreshConsultationLinks(){
    [].slice.call(document.querySelectorAll("a[href]")).forEach(function(link){
      var href=link.getAttribute("href")||"";
      if(href==="/contacts?chat=consultant")link.setAttribute("href","/contacts?consultant=current");
    });
  }
  function gpOpenCurrentConsultant(){
    if(location.pathname.indexOf("/contacts")!==0)return;
    var params=new URLSearchParams(location.search||"");
    var wantsCurrent=params.get("consultant")==="current"||params.get("chat")==="consultant";
    if(!wantsCurrent)return;
    if(params.get("chat")==="consultant"){
      params.delete("chat");params.set("consultant","current");
      try{history.replaceState(null,"",location.pathname+"?"+params.toString());}catch(error){}
    }
    return;
    var root=document.querySelector(".gp-contact-widget-fallback");
    if(!root)return;
    ...
    root.classList.remove("is-chat");
    var main=root.querySelector(".gp-contact-widget-main");
    if(main)main.setAttribute("aria-expanded","true");
    [].slice.call(root.querySelectorAll(".gp-contact-widget-old-title")).forEach(function(node){
      if(/онлайн/i.test(gpFinalText(node)))node.textContent="Написать онлайн-консультанту";
    });
    [].slice.call(root.querySelectorAll(".gp-compare-kicker-v2")).forEach(function(node){node.textContent="Онлайн-консультант";});
  }
  function gpStabilizeContactsMap(){
    if(location.pathname.indexOf("/contacts")!==0)return;
    var frame=document.querySelector('iframe[title^="Карта"]');
    if(!frame||frame.dataset.gpMapEager20260729==="1")return;
    frame.dataset.gpMapEager20260729="1";
    frame.setAttribute("loading","eager");
    frame.setAttribute("fetchpriority","high");
    var source=frame.getAttribute("src")||"";
    if(source)frame.setAttribute("src",source);
  }
  function gpApplyProductContactsDocs(){
    gpEnsureProductContactsDocsStyle();
    gpRefreshConsultationLinks();
    gpCleanProductDetails();
    gpStabilizeContactsMap();
    gpOpenCurrentConsultant();
  }
  gpApplyProductContactsDocs();
  setTimeout(gpApplyProductContactsDocs,0);
  setTimeout(gpApplyProductContactsDocs,700);
  setTimeout(gpApplyProductContactsDocs,1700);
  setTimeout(gpApplyProductContactsDocs,3200);
}catch(error){console.warn("gp product contacts docs fixes failed",error)}
})();;(function(){
  var marker="gp-requisites-icon-remove-v20260729";
  function apply(){
    if(document.getElementById(marker))return;
    var style=document.createElement("style");
    style.id=marker;
    style.textContent="section.gp-requisites-card .gp-requisites-icon,section.gp-requisites-card [data-gp-icon]{display:none!important}section.gp-requisites-card{grid-template-columns:minmax(0,1fr)!important}";
    (document.head||document.documentElement).appendChild(style);
  }
  apply();
  ...
  var marker="gp-requisites-icon-inline-remove-v20260729";
  function apply(){
    document.querySelectorAll("section.gp-requisites-card .gp-requisites-icon,section.gp-requisites-card [data-gp-icon]").forEach(function(icon){
      icon.style.setProperty("display","none","important");
      icon.setAttribute("aria-hidden","true");
      icon.setAttribute("data-gp-hidden",marker);
    });
  }
  apply();
  [0,150,700,1800,3200].forEach(function(delay){setTimeout(apply,delay)});
  window.addEventListener("pageshow",apply,{passive:true});
})();;(function(){
  function openCurrentConsultant(){
    return;
    if(location.pathname.indexOf("/contacts")!==0)return;
    var params=new URLSearchParams(location.search||"");
    if(params.get("consultant")!=="current"&&params.get("chat")!=="consultant")return;
  ...
  [0,700,1700,3200,5000,7000].forEach(function(delay){setTimeout(openCurrentConsultant,delay)});
})();
;(function(){
  var marker="gp-current-consultant-menu-observer-v20260729";
  function wantsCurrent(){
    return false;
    if(location.pathname.indexOf("/contacts")!==0)return false;
    var params=new URLSearchParams(location.search||"");
    return params.get("consultant")==="current"||params.get("chat")==="consultant";
  }
  function installStyle(){
    if(document.getElementById(marker))return;
    var style=document.createElement("style");
    style.id=marker;
    style.textContent=".gp-contact-widget-fallback[data-gp-current-consultant=\"current\"] .gp-contact-widget-old-menu{display:grid!important}.gp-contact-widget-fallback[data-gp-current-consultant=\"current\"] .gp-contact-widget-panel{display:none!important}";
    (document.head||document.documentElement).appendChild(style);
  }
  function apply(){
    if(!wantsCurrent())return;
    ...
    var root=document.querySelector(".gp-contact-widget-fallback");
    if(!root)return;
    root.setAttribute("data-gp-current-consultant","current");
    root.classList.remove("is-chat");
    root.style.removeProperty("display");
    var trigger=root.querySelector(".gp-contact-widget-main");
    if(trigger)trigger.setAttribute("aria-expanded","true");
  ...
  function watch(){
    apply();
    if(!wantsCurrent()||!document.documentElement)return;
    var observer=new MutationObserver(function(mutations){
      if(mutations.some(function(mutation){return mutation.type==="childList"&&mutation.addedNodes&&mutation.addedNodes.length;}))apply();
    });
    observer.observe(document.documentElement,{childList:true,subtree:true});
    setTimeout(function(){observer.disconnect();},20000);
  }
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",watch,{once:true});else watch();
  [250,900,2100,4200,7600,10500,14000,18000].forEach(function(delay){setTimeout(apply,delay)});
  window.addEventListener("pageshow",watch,{passive:true});
})();
;(function(){
  var marker="gp-requisites-icon-observer-v20260729";
  function apply(){
    document.querySelectorAll("section.gp-requisites-card .gp-requisites-icon,section.gp-requisites-card [data-gp-icon]").forEach(function(icon){
      icon.style.setProperty("display","none","important");
      icon.setAttribute("aria-hidden","true");
  ...
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",watch,{once:true});else watch();
  [150,700,1800,3200,5000,7600,10500,14000,18000].forEach(function(delay){setTimeout(apply,delay)});
  window.addEventListener("pageshow",watch,{passive:true});
})();
;(function(){
  var marker="gp-current-consultant-once-v20260729";
  function wantsCurrent(){
    if(location.pathname.indexOf("/contacts")!==0)return false;
    var params=new URLSearchParams(location.search||"");
    if(params.get("chat")==="consultant"){
      params.delete("chat");
      params.set("consultant","current");
      try{history.replaceState(null,"",location.pathname+"?"+params.toString());}catch(error){}
    }
    return params.get("consultant")==="current";
  }
  function installStyle(){
    if(document.getElementById(marker))return;
    var style=document.createElement("style");
    style.id=marker;
    style.textContent=[
      ".gp-contact-widget-old-menu .gp-contact-widget-old-close{position:absolute!important;right:.45rem!important;top:-.88rem!important;z-index:5!important;display:grid!important;width:2.15rem!important;height:2.15rem!important;place-items:center!important;border:1px solid rgba(175,90,102,.22)!important;border-radius:999px!important;background:#fff!important;color:var(--brand-text,#2f2723)!important;box-shadow:0 10px 24px rgba(36,29,25,.14)!important;font-size:1.4rem!important;line-height:1!important;cursor:pointer!important}",
      ".gp-contact-widget-old-menu .gp-contact-widget-old-close:hover{background:#f8e7e9!important;color:var(--brand-primary,#af5a66)!important}",
      "@media(max-width:640px){.gp-contact-widget-old-menu .gp-contact-widget-old-close{right:.35rem!important;top:-.7rem!important;width:2rem!important;height:2rem!important}}"
    ].join("");
    (document.head||document.documentElement).appendChild(style);
  }
  function addClose(menu){
    if(menu.querySelector("[data-gp-current-consultant-close]"))return;
    var close=document.createElement("button");
    close.type="button";
    close.className="gp-contact-widget-old-close";
    close.setAttribute("data-gp-contact-close","");
    close.setAttribute("data-gp-current-consultant-close",marker);
    close.setAttribute("aria-label","Закрыть окно консультанта");
    close.textContent="×";
  ...
  function arm(){
    ...
    function openOnce(){
      if(done)return true;
      var root=document.querySelector(".gp-contact-widget-fallback[data-gp-contact-widget-version='oldvisual73']");
      var menu=root&&root.querySelector(".gp-contact-widget-old-menu[data-gp-final-ux='1']");
      if(!root||!menu)return false;
      root.removeAttribute("data-gp-current-consultant");
      root.classList.add("is-open");
      root.classList.remove("is-chat");
      root.style.removeProperty("display");
      var trigger=root.querySelector(".gp-contact-widget-main");
    ...
    if(openOnce())return;
    observer=new MutationObserver(function(){openOnce();});
    observer.observe(document.documentElement,{childList:true,subtree:true});
...
;(function(){
  var marker="gp-requisites-centered-v20260729";
  function apply(){
    if(!document.head||document.getElementById(marker))return;
    var style=document.createElement("style");
    style.id=marker;
    style.textContent=[
      "body.gp-path-contacts .gp-requisites-card{display:grid!important;grid-template-columns:minmax(0,1fr)!important;width:100%!important;max-width:none!important;margin:1.1rem 0 0!important;padding:1.35rem clamp(1rem,3vw,2rem)!important;box-sizing:border-box!important;justify-items:stretch!important;text-align:center!important}",
      "body.gp-path-contacts .gp-requisites-card>div:not(.gp-requisites-icon){display:block!important;width:100%!important;max-width:none!important;text-align:center!important}",
      "body.gp-path-contacts .gp-requisites-card .gp-requisites-label,body.gp-path-contacts .gp-requisites-card h2{text-align:center!important}",
      "body.gp-path-contacts .gp-requisites-card .gp-legal-details{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:.7rem!important;width:100%!important;max-width:none!important;margin:1rem 0 0!important;text-align:center!important}",
      "body.gp-path-contacts .gp-requisites-card .gp-legal-details>div,body.gp-path-contacts .gp-requisites-card .gp-legal-details>div:first-child{display:flex!important;min-width:0!important;min-height:5.75rem!important;align-items:center!important;justify-content:center!important;flex-direction:column!important;gap:.3rem!important;overflow:hidden!important;border:1px solid rgba(175,90,102,.12)!important;border-radius:1rem!important;background:rgba(255,255,255,.76)!important;padding:.78rem .85rem!important;box-sizing:border-box!important;text-align:center!important}",
      "body.gp-path-contacts .gp-requisites-card .gp-legal-details span{display:block!important;max-width:100%!important;color:var(--brand-muted,#8b7b72)!important;font-size:.7rem!important;font-weight:600!important;letter-spacing:.14em!important;line-height:1.2!important;text-align:center!important;text-transform:uppercase!important}",
      "body.gp-path-contacts .gp-requisites-card .gp-legal-details strong{display:block!important;max-width:100%!important;overflow-wrap:anywhere!important;color:var(--brand-text,#241d19)!important;font-size:clamp(1rem,1.35vw,1.2rem)!important;font-weight:650!important;line-height:1.22!important;text-align:center!important}",
      "body.gp-path-contacts .gp-requisites-card .gp-legal-details>div:not(:first-child) strong{font-size:clamp(1.12rem,1.6vw,1.34rem)!important;letter-spacing:.01em!important}",
      "@media(max-width:767px){body.gp-path-contacts .gp-requisites-card{padding:1.15rem 1rem!important}body.gp-path-contacts .gp-requisites-card .gp-legal-details{grid-template-columns:minmax(0,1fr)!important;gap:.52rem!important}body.gp-path-contacts .gp-requisites-card .gp-legal-details>div,body.gp-path-contacts .gp-requisites-card .gp-legal-details>div:first-child{min-height:0!important;padding:.72rem .85rem!important}}"
    ].join("");
    document.head.appendChild(style);
  }
  apply();
...
;(function(){
  var marker="gp-requisites-inline-layout-v20260729";
  function set(node,property,value){if(node)node.style.setProperty(property,value,"important");}
  function apply(){
    if(location.pathname.indexOf("/contacts")!==0)return;
    var card=document.querySelector("section.gp-requisites-card");
    if(!card)return;
    ...
    set(card,"text-align","center");
    set(card,"padding",desktop?"1.35rem clamp(1rem,3vw,2rem)":"1.15rem 1rem");
    if(desktop){
      set(card,"position","relative");
      set(card,"left","50%");
      set(card,"width","calc(100vw - 3rem)");
      set(card,"max-width","84rem");
      set(card,"margin-left","0");
      set(card,"margin-right","0");
      set(card,"transform","translateX(-50%)");
    }else{
      set(card,"position","relative");
      set(card,"left","auto");
      set(card,"width","100%");
    ...
    [].slice.call(card.querySelectorAll(".gp-requisites-label,h2")).forEach(function(node){set(node,"text-align","center")});
    var details=card.querySelector(".gp-legal-details");
    if(!details)return;
    set(details,"display","grid");
    set(details,"width","100%");
    ...
    [].slice.call(details.children).forEach(function(row,index){
      ...
      set(row,"text-align","center");
      set(row,"min-height",desktop?"5.8rem":"0");
      set(row,"padding",desktop?".8rem .9rem":".74rem .86rem");
      [].slice.call(row.querySelectorAll("span,strong")).forEach(function(node){set(node,"text-align","center")});
      var value=row.querySelector("strong");
      set(value,"font-size",desktop?"clamp(1.18rem,1.7vw,1.42rem)":"1.16rem");
      set(value,"font-weight","650");
      set(value,"line-height","1.18");
    });
  }
  function watch(){
    apply();
  ...
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",watch,{once:true});else watch();
  [250,800,1800,3500,6000,10000,15000,19000].forEach(function(delay){setTimeout(apply,delay)});
  window.addEventListener("pageshow",watch,{passive:true});
})();
;(function(){
  var marker="gp-requisites-full-width-v20260729";
  function apply(){
    if(!document.head||document.getElementById(marker))return;
    var style=document.createElement("style");
    style.id=marker;
    style.textContent=[
      ".gp-path-contacts .gp-requisites-card{display:grid!important;grid-template-columns:minmax(0,1fr)!important;width:100%!important;max-width:none!important;margin:1.1rem 0 0!important;padding:1.35rem clamp(1rem,3vw,2rem)!important;box-sizing:border-box!important;justify-items:stretch!important;text-align:center!important}",
      ".gp-path-contacts .gp-requisites-card>div:not(.gp-requisites-icon){display:block!important;width:100%!important;max-width:none!important;text-align:center!important}",
      ".gp-path-contacts .gp-requisites-card .gp-requisites-label,.gp-path-contacts .gp-requisites-card h2{text-align:center!important}",
      ".gp-path-contacts .gp-requisites-card .gp-legal-details{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:.72rem!important;width:100%!important;max-width:none!important;margin:1rem 0 0!important;text-align:center!important}",
      ".gp-path-contacts .gp-requisites-card .gp-legal-details>div{display:flex!important;min-width:0!important;min-height:5.8rem!important;align-items:center!important;justify-content:center!important;flex-direction:column!important;gap:.32rem!important;overflow:hidden!important;border:1px solid rgba(175,90,102,.12)!important;border-radius:1rem!important;background:rgba(255,255,255,.76)!important;padding:.8rem .9rem!important;box-sizing:border-box!important;text-align:center!important}",
      ".gp-path-contacts .gp-requisites-card .gp-legal-details>div:first-child{display:none!important}",
      ".gp-path-contacts .gp-requisites-card .gp-legal-details span{display:block!important;max-width:100%!important;color:var(--brand-muted,#8b7b72)!important;font-size:.72rem!important;font-weight:600!important;letter-spacing:.15em!important;line-height:1.2!important;text-align:center!important;text-transform:uppercase!important}",
      ".gp-path-contacts .gp-requisites-card .gp-legal-details strong{display:block!important;max-width:100%!important;overflow-wrap:anywhere!important;color:var(--brand-text,#241d19)!important;font-size:clamp(1.18rem,1.7vw,1.42rem)!important;font-weight:650!important;line-height:1.18!important;letter-spacing:.01em!important;text-align:center!important}",
      "@media(min-width:768px){.gp-path-contacts .gp-requisites-card{position:relative!important;left:50%!important;width:min(calc(100vw - 3rem),84rem)!important;max-width:min(calc(100vw - 3rem),84rem)!important;margin-left:0!important;margin-right:0!important;transform:translateX(-50%)!important}}",
      "@media(max-width:767px){.gp-path-contacts .gp-requisites-card{padding:1.15rem 1rem!important}.gp-path-contacts .gp-requisites-card .gp-legal-details{grid-template-columns:minmax(0,1fr)!important;gap:.54rem!important}.gp-path-contacts .gp-requisites-card .gp-legal-details>div{min-height:0!important;padding:.74rem .86rem!important}}"
    ].join("");
    document.head.appendChild(style);
  }
  apply();
